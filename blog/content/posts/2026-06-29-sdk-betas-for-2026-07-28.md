...
The betas exist so you can test the protocol changes and give us feedback
before the specification is final. For production, the stable releases
remain the recommended versions. After v2 ships, the TypeScript SDK will
keep shipping v1.x bug fixes and security updates for at least six months,
and the Python v1.x line continues to receive critical bug fixes and
security patches.

Trying a beta is safe because it is opt-in at every step. Installing any of
the four SDKs without explicitly requesting a pre-release still resolves to
...
- When you test a beta, pin the exact version. Public APIs may still change
  between the betas and the stable releases.
