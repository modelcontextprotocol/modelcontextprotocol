...
Picture a remote MCP server in production today. It sits behind a load balancer,
but the load balancer is configured for sticky sessions because the `initialize`
handshake and the `Mcp-Session-Id` header tie every subsequent request to the
instance that handled the first one. A gateway in front of it wants to
rate-limit `tools/call` separately from `tools/list`, but it can't tell them
apart without parsing the JSON-RPC body. An orchestrator running fifty subagents
against that server makes fifty separate `tools/list` calls because it has no
idea whether the result is safe to reuse across callers.

After this release, that same server runs behind a plain round-robin load
balancer. The gateway routes on an `Mcp-Method` header. The orchestrator caches
one `tools/list` response for as long as the server's `ttlMs` says it can.
...
[2026 roadmap](/posts/2026-mcp-roadmap/): a stateless core that scales on
ordinary HTTP infrastructure, server-rendered UIs through
[MCP Apps](#mcp-apps-server-rendered-user-interfaces), long-running work through
the [Tasks extension](#tasks-graduates-to-an-extension), and authorization that
aligns more closely with OAuth and OpenID Connect deployments.
...
stateless. Servers that need to carry state across calls do it the way HTTP APIs
always have: mint an explicit handle (a `basket_id`, a `browser_id`) from a tool
and have the model pass it back as an ordinary argument on later calls. The
protocol no longer manages that state for you, but it doesn't prevent you from
managing it yourself, and the explicit-handle pattern makes the state visible to
the model rather than hidden in transport metadata.
...
which is the same suite the new
[SDK tier system](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/1777)
