...
For a comprehensive guide on running MCP servers in different modes (stdio vs HTTP/SSE), see our [Running MCP Servers](/quickstart/running-mcp-servers) guide.
...
**Note**: These servers run in stdio mode by default. For HTTP/SSE transport (port-based connections), see the [Running MCP Servers](/quickstart/running-mcp-servers) guide.
