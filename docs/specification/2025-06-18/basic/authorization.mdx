...
(a) a user-facing input mechanism for a pre-provisioned `client_id` (and `client_secret` for confidential clients), **or**
(b) a managed configuration mechanism (e.g., OS-standard configuration file).
