...
1. A user-facing input mechanism for a pre-provisioned `client_id` (and `client_secret` for confidential clients), or
2. A managed configuration mechanism (e.g., an OS-standard configuration file).
...
**Authorization servers** that do not support DCR **MUST** provide a non-DCR mechanism to obtain a `client_id` (and credentials for confidential clients), such as an administrative portal.
...
**Pre-registration: operational risks and mitigations**

Pre-registration can cause client-secret sprawl and lifecycle/rotation risks; implementations **SHOULD** prefer public clients where feasible.
