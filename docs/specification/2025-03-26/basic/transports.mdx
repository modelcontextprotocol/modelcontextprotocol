...
<Note>
  Implementation considerations: If a server keeps session state in process memory,
  deployments should ensure requests carrying a given `Mcp-Session-Id` are routed to
  the same backend (session affinity) or externalize session state so any worker can
  handle the request. This is especially relevant for session-bound features like
  elicitation or task-augmented flows.
</Note>
