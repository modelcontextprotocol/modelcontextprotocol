...
<Note>
  Implementation considerations: If elicitation state is stored in process memory,
  deployments should ensure requests for a given `Mcp-Session-Id` are routed to the
  same backend (session affinity) or externalize state so any worker can handle the
  request.
</Note>
