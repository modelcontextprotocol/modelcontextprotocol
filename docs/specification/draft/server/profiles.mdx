...
Anyone can create a profile, and can maintain it at a Profile Specification URL of their choice. The MCP community creates and maintains standard profiles.
...
### Using Profiles
MCP Servers can declare the profiles they support by referring to their Profile Specification URL. MCP Clients discover which profiles an MCP Server supports and select the right profiles to use. The following section describes how all this happens.
...
## Profile Discovery
Before connecting to an MCP Server, an MCP Client MAY fetch the Server’s Well Known URL to understand which profiles the server supports (if any). Alternatively, an MCP Client may, through proprietary means, already know the profiles a specific MCP Server supports and simply use those.
...
The following diagram outlines the selection flow that is described below the diagram:
```mermaid
  sequenceDiagram
      participant C as MCP Client
      participant S as MCP Server

      Note over C,S: Profile Selection Process
