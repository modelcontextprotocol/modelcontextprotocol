...
Completion values can be objects with `title`, `value`, and optional `description` fields. The `description` field provides additional context about the completion option.

For simple use cases (where the `value` is the same as the `title`), the
