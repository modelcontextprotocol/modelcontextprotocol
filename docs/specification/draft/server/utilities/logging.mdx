...
### Long-lived log stream

To receive a continuous stream of log messages, send a
[`subscriptions/listen`][subscriptions] request with `logLevel` set in the
`notifications` filter. The server delivers `notifications/message` on that stream
for the duration of the subscription.

See [Subscriptions][subscriptions] for the full protocol mechanics.
