...
Servers **MAY** also respond to `resources/read` with an [`IncompleteResult`](/specification/draft/basic#incompleteresult) to indicate that additional input is needed before the resource can be read. This follows the [multi round-trip requests](/specification/draft/basic#multi-round-trip-requests) mechanism. When retrying the request, clients include `inputResponses` and, if provided by the server, `requestState` in the request parameters.
...
Clients **SHOULD** send `resources/unsubscribe` when they no longer need updates
for a resource.
