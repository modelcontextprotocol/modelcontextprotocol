...
The response includes all resource metadata fields from the `Resource` type, along with the content (`text` or `blob`).

### Reading Resource Metadata

Clients can request only metadata without fetching the actual content using the `resources/metadata` request:
...
This allows clients to efficiently check resource metadata (e.g., to verify if a resource has been modified via the `lastModified` annotation) without fetching potentially large contents.
