...
The `contents` array in `ReadResourceResult` **SHOULD** contain multiple elements only when
returning the same resource in different MIME types. This allows clients to choose the most
appropriate format based on their capabilities.
...
**Example:** A PDF document returned in multiple formats:
...
In this example, clients with PDF rendering capabilities can use the `application/pdf` blob,
while clients without such capabilities can fall back to the extracted `text/plain` content.
