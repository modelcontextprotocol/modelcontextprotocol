...
The protocol supports optional subscriptions to resource changes. The subscription is associated with a particular MCP session rather than a particular connection. Clients can subscribe to specific resources and receive notifications when they change:
