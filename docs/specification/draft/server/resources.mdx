...
`listChanged` is optional&mdash;servers may omit it:
...
Clients can subscribe to change notifications for specific resources using
[`subscriptions/listen`](/specification/draft/schema#subscriptionslistenrequest).
The request opens a long-lived stream; the server delivers
`notifications/resources/updated` on that stream whenever a watched resource changes.
...
The server **MUST** send a `notifications/subscriptions/acknowledged` notification
to confirm the stream is active before sending any other notifications on it:
...
**Update notification** (delivered on the stream):
...
To end the subscription, close the stream (HTTP) or send
`notifications/cancelled` referencing the listen request ID (stdio). See
[Cancellation](/specification/draft/basic/utilities/cancellation) for details.
