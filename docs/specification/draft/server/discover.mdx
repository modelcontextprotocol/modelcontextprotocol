...
| `ttlMs`             | `number`             | yes      | How long (ms) the client MAY consider this result fresh. See [Caching](/specification/draft/server/utilities/caching).          |
| `cacheScope`        | `string`             | yes      | `"public"` or `"private"`. Indicates the intended cache audience. See [Caching](/specification/draft/server/utilities/caching). |
