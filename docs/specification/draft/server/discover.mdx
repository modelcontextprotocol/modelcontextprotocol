...
- **stdio backward-compatibility probe.** On stdio, there is no per-request
  ...
  send `server/discover` first. A `DiscoverResult` or a recognized modern
  error (such as
  [`UnsupportedProtocolVersionError`](/specification/draft/schema#unsupportedprotocolversionerror))
  identifies a modern server. Any other error — or no response within a
  reasonable timeout — identifies a legacy server, and the client falls back
  to the `initialize` handshake. The fallback **MUST NOT** be keyed to one
  specific error code, because legacy servers respond to unknown
