...
Clients **SHOULD** respect the timeout declared by the server in its `prompts.timeouts.list`
...
Clients **SHOULD** respect the timeout declared by the server in its `prompts.timeouts.get`
capability, or use the protocol default of 15 seconds if not specified.
