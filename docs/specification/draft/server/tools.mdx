...
For long-running tools, servers MAY stream intermediate progress notifications and
partial result chunks during `tools/call` execution, providing visibility into progress
before the final result arrives.

#### Capabilities

Servers that support streaming tool results MUST declare `streaming: true` in their
`tools` capability:
...
During a `tools/call`, the server sends one or more
`notifications/tool/progress` notifications before the final `CallToolResult`:

**Progress with content chunk:**
...
`requestId` is the JSON-RPC `id` from the originating `tools/call` request and is the
primary correlation key — it disambiguates concurrent calls to the same tool.
`toolName` is informational only.

The `contentChunk` field is optional and follows the same
[ContentBlock](#content-types) types used in the final result. The `seq` field is
**required** (when `contentChunk` is present) and provides a monotonic ordering
guarantee — each notification for the same `requestId` MUST increment `seq` by
exactly 1. Clients MUST use `seq` to reorder chunks before rendering.

The final `CallToolResult` is the authoritative response. Clients MUST replace
any progressively rendered content with the final result when it arrives.
...
sequenceDiagram
    ...
    Client->>Server: tools/call (id: 2)
    Server--)Client: notifications/tool/progress (requestId: 2, seq: 0)
    Server--)Client: notifications/tool/progress (requestId: 2, seq: 1)
...
(standard JSON-RPC behavior). Clients that support streaming MUST:

1. **Correlate** notifications to active tool calls using `requestId`
2. **Order** chunks by `seq` (strictly monotonic per `requestId`)
3. **Discard** notifications for unknown or already-completed `requestId` values
4. **Render** content progressively as chunks arrive
5. **Replace** with the authoritative `CallToolResult` when received
