...
<Note>
  [MCP Apps](https://modelcontextprotocol.io/docs/extensions/apps) hosts typically pass `content`
  to the model and use `structuredContent` or `_meta` for UI rendering or host-specific metadata.
  Apps can update model context explicitly via `ui/update-model-context`.
</Note>
