...
Clients **SHOULD** respect the timeout declared by the server in its `tools.timeouts.list`
...
Clients **SHOULD** respect the timeout declared by the server in its `tools.timeouts.call`
capability, or use the protocol default of 60 seconds if not specified.
