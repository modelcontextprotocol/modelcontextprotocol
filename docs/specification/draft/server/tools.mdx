...
- `execution`: Optional object describing execution-related properties
  - `taskSupport`: **Deprecated.** This field is deprecated and will be removed in a future version. Receivers MAY return `CreateTaskResult` in response to any request, regardless of this field. Values: `"forbidden"` (default), `"optional"`, or `"required"`
