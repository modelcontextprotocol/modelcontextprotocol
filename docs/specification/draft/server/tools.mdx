...
The `content` and `structuredContent` fields serve different audiences:

- `content`: model-oriented output optimized for readability and token efficiency. Preferred for conversational agents and when tool responses are fed directly into the model's context window.
- `structuredContent`: machine-oriented output for programmatic use, code generation, typed orchestration, and UI hydration.

When both fields are present, they SHOULD be semantically equivalent (same information, different presentation). `content` may summarize or textualize the structured data without listing every field verbatim, but it SHOULD NOT contradict it.

For backwards compatibility, tools that return structured content SHOULD also return a `content` representation of the same information. JSON-serialized text is acceptable, but not required and may be suboptimal depending on the use case.
...
fall back to `structuredContent` if `content` is empty or omitted. Programmatic/code-mode hosts
that want structured inputs can omit `content` so the fallback applies.

Clients SHOULD NOT forward both fields verbatim to the model as separate inputs.

<Note>
  [MCP Apps](https://modelcontextprotocol.io/docs/extensions/apps) hosts
  typically pass `content` to the model and use `structuredContent` or `_meta`
  for UI rendering or host-specific metadata. Apps can update model context
  explicitly via `ui/update-model-context`.

Implementers SHOULD ensure `content` still includes a semantically equivalent result
that will be visible to the model, even when the format is more concise or tailored
than the `structuredContent` response used to populate UI controls.

</Note>

Example MCP Apps-oriented tool result:
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "DHL Order #123 is currently in transit. Estimated delivery: 2026-02-18, departed sorting facility at 2026-02-15T08:32:00Z"
      }
    ],
    "structuredContent": {
      "orderId": "123",
...
This keeps model-relevant facts in `content` while preserving UI-specific detail for hosts.
...
- For non-error results (`isError` absent or `false`), servers **MUST** provide
  `structuredContent` that conforms to this schema.
- For error results (`isError: true`), servers **MAY** omit
  `structuredContent`.
- Clients **SHOULD** validate `structuredContent` against this schema for
  non-error results.
- Clients **SHOULD NOT** validate `structuredContent` against this schema when
  `isError` is `true`.
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "Temperature: 22.5 C, conditions: partly cloudy, humidity: 65%."
      }
    ],
    "structuredContent": {
      "temperature": 22.5,
...
When returning both fields, `structuredContent` should carry the full structured data required by
`outputSchema`, while `content` should provide a concise, model-friendly equivalent.
...
When reporting tool execution errors, servers **SHOULD** return the actionable
