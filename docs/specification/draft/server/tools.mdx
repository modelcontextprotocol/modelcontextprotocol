...
- `content`: model-oriented output optimized for readability and token efficiency. Preferred for conversational agents and direct model prompting.
...
For backwards compatibility, tools that return structured content SHOULD also return a `content` representation of the same information. JSON-serialized text is acceptable, but not required.
...
`outputSchema`, while `content` should provide a concise, model-friendly summary.
