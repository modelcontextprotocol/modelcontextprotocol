...
- `execution`: Optional object describing execution-related properties
  - `taskSupport`: Values: `"forbidden"` (default), `"optional"`, or `"required"`. **Deprecated:** this field will be removed in a future specification version; receivers MAY return `CreateTaskResult` regardless of this field.
