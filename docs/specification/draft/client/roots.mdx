...
<Warning>
  **Deprecated**: The Roots feature is deprecated as of protocol version
  ...
  Under the [feature lifecycle
  policy](/seps/2596-spec-feature-lifecycle-and-deprecation), it remains in the
  specification for at least twelve months after this revision's release before
  it becomes eligible for removal. New implementations **SHOULD NOT** adopt it;
  existing implementations **SHOULD** migrate to passing directories or files
  via tool parameters, resource URIs, or server configuration. See the
