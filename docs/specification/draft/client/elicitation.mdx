...
Servers **MAY** request information from a user during the processing of a client request, by sending an [`InputRequiredResult`](/specification/draft/basic/transports/mrtr#inputrequiredresult)
...
Elicitations do not require that the server maintain state about users with the [multi round-trip requests](/specification/draft/basic/transports/mrtr#multi-round-trip-requests) mechanism.
