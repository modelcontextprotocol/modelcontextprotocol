...
When another request cannot be processed until an elicitation is completed, the server **SHOULD** return an [ElicitationRequired error](#error-handling) (code `-32604`) to indicate to the client that an elicitation is required.
...
For servers that interact with APIs requiring sensitive credentials (e.g. LLM APIs), URL mode elicitation provides a secure mechanism for collecting API keys without exposing them to the MCP client or host application.
...
The "downstream authorization" enabled by URL mode elicitation is fundamentally separate from [MCP authorization](../basic/authorization).

<Note>
  This distinction is critical to understand. URL mode elicitation is *not* about
...
- When a request cannot be processed until an elicitation is completed: `-32604` (ElicitationRequired)
