...
Servers **MAY** request information from a user during the processing of a client request, by sending an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult)
...
sequenceDiagram
    ...
    Server-->>Client: IncompleteResult(elicitation/create (mode: form))
...
sequenceDiagram
    ...
    Client->>Server: tools/call(id: 1)
    Note right of Server: Server needs more info that requires URL mode elicitation. <br/> Server creates requestState that encodes information about original request and the URL that the user will need to visit to complete the elicitation.
    Server-->>Client: IncompleteResult(elicitation/create (mode: url), requestState)
    ...
    Note right of Server: Server uses requestState to to associate the user's response via the URL with this request. It may need to block until the request is fulfilled.
...
sequenceDiagram
    ...
    Server->>Client: IncompleteResult<br/>(mode: "url", url: "https://mcp.example.com/connect?..."), requestState
