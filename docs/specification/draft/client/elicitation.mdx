...
1. Servers **MUST** bind elicitation requests to the user's identity
...
1. **MUST NOT** open a user agent (e.g. browser) without explicit consent from the user.
2. **MUST** render the URL provided by the server in a browser which does not enable the client or LLM to inspect the content or user inputs.
