...
7. Loosen `inputSchema` and `outputSchema` to allow any JSON Schema 2020-12 keywords, and
...
8. Require MCP clients to validate the `iss` parameter in authorization responses per
   [RFC 9207](https://datatracker.ietf.org/doc/html/rfc9207), and recommend authorization
   servers emit it, to mitigate mix-up attacks
