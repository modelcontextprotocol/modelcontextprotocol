...
3. Extend `resources/read` responses to include full resource metadata by having `TextResourceContents` and `BlobResourceContents` extend `Resource`. Add new `resources/metadata` endpoint to fetch metadata without contents ([SEP-1533](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1533)).
