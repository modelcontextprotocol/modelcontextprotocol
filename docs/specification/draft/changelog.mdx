...
5. Clarify the intended audiences for `content` and `structuredContent` in tool results, including model visibility, fallback behavior, and `outputSchema` validation for non-error results ([SEP-2200](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2200)).
