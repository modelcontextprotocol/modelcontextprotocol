...
Extensions are negotiated during initialization through the `extensions` field within client and server capabilities. Each extension is identified by a unique identifier in the format `{vendor-prefix}/{extension-name}` (e.g., `modelcontextprotocol.io/official-extension`).

### Extension Types

MCP recognizes three categories of extensions:

1. **Official Extensions**: Maintained in the [modelcontextprotocol GitHub organization](https://github.com/modelcontextprotocol) under repositories with the `ext-` prefix. These extensions undergo review by core maintainers and follow the same governance as the core protocol.

2. **Recommended Extensions**: Community-developed extensions that have been vetted and are listed on the [official extensions registry](/extensions). While not officially maintained, these extensions meet quality and compatibility standards.

3. **Unofficial Extensions**: Developed independently without MCP oversight. These extensions use vendor-specific prefixes and may be used at implementers' own discretion.
