...
Servers can also include a `scope` parameter in the `WWW-Authenticate` challenge; the scope
semantics and the associated client behavior are defined in the
[Scope Selection Strategy](/specification/draft/basic/authorization#scope-selection-strategy) section.
...
Metadata and OpenID Connect Discovery 1.0, MCP clients **MUST**
attempt multiple well-known endpoints when discovering
authorization server metadata. Well-known URL construction is
defined in
[RFC 8414 Section 3.1](https://datatracker.ietf.org/doc/html/rfc8414#section-3.1)
and
...
covers the interoperability between the two.
...
(e.g., `https://auth.example.com`), clients **MUST** try the
`oauth-authorization-server` well-known URI first, then
`openid-configuration`, each at the root of the issuer URL.
