...
- The metadata document **MUST** include at least the following properties: `client_id`, `client_name`, `redirect_uris`, `token_endpoint_auth_methods_supported`
- Clients **MUST** ensure the `client_id` value in the metadata matches the document URL exactly
- Clients **MUST** include `token_endpoint_auth_methods_supported`, a JSON array of one or more token endpoint authentication method names drawn from the [OAuth Token Endpoint Authentication Methods registry](https://www.iana.org/assignments/oauth-parameters/oauth-parameters.xhtml#token-endpoint-auth-method) ([RFC 7591 Section 4.2](https://datatracker.ietf.org/doc/html/rfc7591#section-4.2)), in no particular order of preference. The array **MUST NOT** include `client_secret_post`, `client_secret_basic`, `client_secret_jwt`, or any other method based around a shared symmetric secret.
...
- **MUST** validate the client uses an accepted authentication method from `token_endpoint_auth_methods_supported`
