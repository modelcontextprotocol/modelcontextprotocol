...
capabilities through authorization server metadata discovery. The normative discovery
requirements are defined in
[Authorization Server Discovery](/specification/draft/basic/authorization/authorization-server-discovery).

## Client Registration

Before initiating the authorization flow, MCP clients obtain a client ID through one of
three registration mechanisms: Client ID Metadata Documents, pre-registration, or Dynamic
Client Registration. The mechanisms and their selection priority are defined in
