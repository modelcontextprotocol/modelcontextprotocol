...
See [Authorization Server Discovery](/specification/draft/basic/authorization/server-discovery) for the mechanisms by which MCP servers advertise their associated authorization servers to MCP clients, as well as the discovery process through which MCP clients can determine authorization server endpoints and supported capabilities.
