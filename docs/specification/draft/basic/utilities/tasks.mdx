...
When a task reaches the `input_required` status and `tasks/result` returns an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult), the requestor provides the required input via a [`tasks/input_response`](/specification/draft/schema#tasks%2Finput_response) request. This is the persistent workflow form of [multi round-trip requests](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests).
