...
Tasks enable asynchronous request processing where receivers return task handles at their discretion for long-running operations. Requestors are responsible for polling for the results of those tasks, while receivers control task creation and manage the lifecycles of those tasks.

This approach enables sophisticated patterns such as dispatching concurrent requests and handling operations that require persistent work, which only the requestor has sufficient context to orchestrate.
...
Servers and clients that support task operations **MUST** declare a `tasks` capability during initialization.
...
<Warning>
  The `tasks.requests` capability is deprecated and will be removed in a future
  specification version. Receivers MAY return `CreateTaskResult` in response to
  any request, regardless of this capability.
</Warning>
...
<Warning>
  The `capabilities.tasks.requests` field is deprecated and will be removed in a
  future specification version. Receivers MAY return `CreateTaskResult` in
  response to any request, regardless of whether the corresponding capability
  has been declared.
</Warning>

During the initialization phase, both parties exchange their `tasks` capabilities.
...
Whether a task is created in response to a request is subject to the receiver's implementation; requestors **MUST** be prepared to handle either case. The requestor need not specify that task-augmented execution is expected.

Requestors **MAY** include a `task.ttl` value indicating the desired task lifetime duration (in milliseconds) since its creation.
...
These requirements apply to all parties involved in task-augmented request handling.
...
1. Receivers **MAY** return a `CreateTaskResult` in response to any request, whether or not the `task` parameter is present in the request.
1. Receivers that receive a `task` parameter in a request and do not wish to create a task **MUST** ignore the parameter.
1. Receivers **MUST NOT** return a `CreateTaskResult` unless and until a `tasks/get` request would return that task; that is, in eventually-consistent systems, receivers **MUST** wait for consistency.
...
This diagram shows the standard task flow where the requestor explicitly includes a task parameter with a TTL in their request.

```mermaid
sequenceDiagram
    participant C as Client (Requestor)
    participant S as Server (Receiver)
    Note over C,S: 1. Task Creation (With Task Parameter)
    C->>S: Request with task field (ttl)
    activate S
    Note over S: Receiver accepts task-augmented request
...
This diagram shows the unsolicited task flow where the requestor sends a regular request without a task field, and the receiver decides to create a task anyway.

```mermaid
sequenceDiagram
    participant C as Client (Requestor)
    participant S as Server (Receiver)
    Note over C,S: 1. Regular Request (No Task Field)
    C->>S: tools/call (no task parameter)
    activate S
    Note over S: Receiver decides to create task<br/>(e.g., operation will take too long)
    S->>C: CreateTaskResult (taskId, status: working, ttl, pollInterval)
    deactivate S
    Note over C: Requestor must handle unexpected task
    Note over C,S: 2. Task Polling (Standard Semantics)
    C->>S: tasks/get (taskId)
    activate S
    S->>C: working
    deactivate S
    ...
    C->>S: tasks/result (taskId)
    activate S
    S->>C: Result content (CallToolResult)
    deactivate S
    Note over C,S: 4. Cleanup
    Note over S: After ttl period from creation, task is cleaned up
```
