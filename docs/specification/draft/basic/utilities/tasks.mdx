...
1. Tasks **MUST** begin in the `pending` status when created.
1. Tasks **MUST** transition to `working` status as soon as execution begins.
1. Receivers **MUST** only transition tasks through the following valid paths:
   1. From `pending`: may move to `working` or `cancelled`
...
- `pending`: The request has been accepted but execution has not yet started.
