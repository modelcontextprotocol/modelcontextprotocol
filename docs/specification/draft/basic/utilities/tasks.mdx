...
Tasks enable asynchronous request processing where receivers can choose to return task handles for long-running operations. Requestors are responsible for polling for the results of those tasks, while receivers control task creation and manage the lifecycles of those tasks.
...
<Warning>
  The `tasks.requests` capability is deprecated. Servers MAY return
  `CreateTaskResult` in response to any request.
</Warning>
...
<Warning>
  The `tasks.requests` capability is deprecated. Clients MAY return
  `CreateTaskResult` in response to any request.
</Warning>
...
sequenceDiagram
    ...
    Note over C,S: 1. Task Creation (Requestor-Initiated)
