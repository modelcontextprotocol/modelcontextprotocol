...
- **Task-augmented requests**: The server accepts the request and immediately returns a `CreateTaskResult` containing task data. The actual operation result becomes available later through `tasks/continue` after the task completes.
...
Requestors can retrieve task status by sending [`tasks/get`](/specification/draft/schema#tasks%2Fget) requests. This operation returns the current task state without requiring the requestor to handle any pending input requests.
...
Requestors advance the task lifecycle by sending [`tasks/continue`](/specification/draft/schema#tasks%2Fcontinue) requests.
Requestors **SHOULD** respect the `pollInterval` provided in responses when determining polling frequency.

Requestors **SHOULD** continue polling until the task reaches a terminal status (`completed`, `failed`, or `cancelled`).
...
**Response (while task is still working):**
...
When a task reaches a terminal status (`completed`, `failed`, or `cancelled`), the final result or error is included directly in the task object returned by `tasks/continue`. For completed tasks, the `result` field contains the operation result. For failed tasks, the `error` field contains the error details.
...
The notification includes the full [`Task`](/specification/draft/schema#task) object, including the updated `status`, `statusMessage` (if present), and `result` or `error` fields (if the task is in a terminal status). This allows requestors to access the complete task state and final result without making additional requests.

Requestors **MUST NOT** rely on receiving this notifications, as it is optional. Receivers are not required to send status notifications and may choose to only send them for certain status transitions. Requestors **SHOULD** continue to poll via `tasks/continue` to ensure they receive status updates.
...
1. When a task reaches the `completed` status, receivers **MUST** include the final result in the `result` field of the task object returned by `tasks/get`, `tasks/continue`, and `notifications/tasks/status`.
1. When a task reaches the `failed` status, receivers **MUST** include the error details in the `error` field of the task object returned by `tasks/get`, `tasks/continue`, and `notifications/tasks/status`.
1. The `result` field structure **MUST** match the result type of the original request (e.g., `CallToolResult` for `tools/call`).
1. The `error` field structure **MUST** match the JSON-RPC error structure that would have been returned if the request had failed without task augmentation.
...
1. For the `tasks/get`, `tasks/continue`, and `tasks/cancel` operations, the `taskId` parameter in the request **MUST** be used as the source of truth for identifying the target task. Requestors **SHOULD NOT** include `io.modelcontextprotocol/related-task` metadata in these requests, and receivers **MUST** ignore such metadata if present in favor of the RPC method parameter.
   Similarly, for the `tasks/get`, `tasks/continue`, `tasks/list`, and `tasks/cancel` operations, receivers **SHOULD NOT** include `io.modelcontextprotocol/related-task` metadata in the result messages, as the `taskId` is already present in the response structure.
...
sequenceDiagram
    ...
    Note over C,S: Server sends elicitation request
...
sequenceDiagram
    ...
    S->>C: tasks/continue (request-789)
    activate C
    C->>S: working
    deactivate C
...
- `result`: The final result of the operation (only present when status is `completed`). The structure matches the result type of the original request.
- `error`: The error that occurred during execution (only present when status is `failed`). The structure matches the JSON-RPC error structure.
...
- `failed`: The associated request did not complete successfully. For tool calls specifically, this includes cases where the tool call result has `isError` set to true. Error details are available in the `error` field.
...
For the `tasks/get`, `tasks/continue`, `tasks/list`, and `tasks/cancel` operations, requestors and receivers **SHOULD NOT** include this metadata in their messages, as the `taskId` is already present in the message structure.
...
- Invalid or nonexistent `taskId` in `tasks/get`, `tasks/continue`, or `tasks/cancel`: `-32602` (Invalid params)
...
When the underlying request does not complete successfully, the task moves to the `failed` status. This includes JSON-RPC protocol errors during request execution, or for tool calls specifically, when the tool result has `isError` set to true. The task object **SHOULD** include a `statusMessage` field with diagnostic information about the failure, and **MUST** include an `error` field with the full error details.
...
For tasks that wrap tool call requests, when the tool result has `isError` set to `true`, the task should reach `failed` status and include the appropriate error details in the `error` field.
