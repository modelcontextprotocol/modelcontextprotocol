...
1. **Base task data** (`Task`): Contains task metadata only — `taskId`, `status`, `statusMessage`, `createdAt`, `lastUpdatedAt`, `ttl`, and `pollInterval`. This shape is used by `CreateTaskResult` and `ListTasksResult`, which **MUST NOT** include `result`, `error`, or `inputRequests` fields.
...
This separation ensures that `CreateTaskResult` and `ListTasksResult` remain lightweight, while `tasks/get` provides the full task state including inlined results and pending requests.
