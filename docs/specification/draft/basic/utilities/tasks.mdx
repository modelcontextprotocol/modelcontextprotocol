...
Task support requires no capability negotiation. Servers **MAY** return `CreateTaskResult` for any request without declaring a `tasks` capability, and clients **MUST** be prepared to handle tasks regardless of declared capabilities.
...
1. For failures, the `error` field **MUST** contain the JSON-RPC error that occurred during execution. The `failed` status **MUST NOT** be used to represent non-JSON-RPC errors, such as a tool result that completed with `isError: true` (which should use the `completed` status with the error in the `result` field).
