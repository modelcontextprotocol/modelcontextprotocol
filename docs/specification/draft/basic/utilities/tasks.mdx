...
Servers have flexibility in how they manage SSE streams during task polling and result retrieval, and clients **SHOULD** expect messages to be delivered on any SSE stream opened via a POST response.
...
1. When the requestor encounters the `input_required` status, it **SHOULD** preemptively call `tasks/result`. If the result is an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult), the requestor **SHOULD** fulfill the `inputRequests` and submit responses via [`tasks/input_response`](#providing-input-for-tasks).
...
1. When a receiver receives a `tasks/result` request for a task in `input_required` status, it **MUST** return an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult) containing the `inputRequests` that the requestor must fulfill via [`tasks/input_response`](#providing-input-for-tasks).
