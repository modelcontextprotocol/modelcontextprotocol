...
When a task is in `input_required` status, `tasks/result` returns an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult) containing `inputRequests` that the client must fulfill via a [`tasks/input_response`](#providing-input-for-tasks) request. See [multi round-trip requests](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests) for details on the persistent workflow.
...
When a task reaches the `input_required` status and `tasks/result` returns an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult), the requestor provides the required input via a [`tasks/input_response`](/specification/draft/schema#tasks%2Finput_response) request. This is the task workflow form of [multi round-trip requests](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests).
...
After sending `tasks/input_response`, the requestor **SHOULD** resume polling via `tasks/get`.
...
1. When the requestor encounters the `input_required` status, it **SHOULD** preemptively call `tasks/result`. If the result is an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult), the requestor **SHOULD** fulfill the `inputRequests` and submit responses via [`tasks/input_response`](#providing-input-for-tasks).
...
1. When a receiver receives a `tasks/result` request for a task in `input_required` status, it **MUST** return an [`IncompleteResult`](/specification/draft/basic/utilities/mrtr#incompleteresult) containing the `inputRequests` that the requestor must fulfill via [`tasks/input_response`](#providing-input-for-tasks).
...
sequenceDiagram
    ...
    activate S
    S->>C: IncompleteResult(elicitation/create)
    activate C
