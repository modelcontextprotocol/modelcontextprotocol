...
When a task is in `input_required` status, `tasks/result` returns an [`IncompleteResult`](/specification/draft/basic#incompleteresult) containing `inputRequests` that the client must fulfill via a [`tasks/input_response`](#providing-input-for-tasks) request. See [multi round-trip requests](/specification/draft/basic#multi-round-trip-requests) for details on the persistent workflow.
...
When a task reaches the `input_required` status and `tasks/result` returns an [`IncompleteResult`](/specification/draft/basic#incompleteresult), the requestor provides the required input via a [`tasks/input_response`](/specification/draft/schema#tasks%2Finput_response) request. This is the persistent workflow form of [multi round-trip requests](/specification/draft/basic#multi-round-trip-requests).

The `tasks/input_response` request carries:
...
- `requestState` (optional): the opaque state token from the `IncompleteResult`, if provided
...
After sending `tasks/input_response`, the task transitions back to `working` status. The requestor **SHOULD** then resume polling via `tasks/get` or call `tasks/result` to retrieve the final result.

Receivers **MUST** reject `tasks/input_response` requests for tasks that are not in `input_required` status with error code `-32602` (Invalid params).
...
1. When the requestor encounters the `input_required` status, it **SHOULD** preemptively call `tasks/result`. If the result is an [`IncompleteResult`](/specification/draft/basic#incompleteresult), the requestor **MUST** fulfill the `inputRequests` and submit responses via [`tasks/input_response`](#providing-input-for-tasks).
...
1. When a receiver receives a `tasks/result` request for a task in `input_required` status, it **MUST** return an [`IncompleteResult`](/specification/draft/basic#incompleteresult) containing the `inputRequests` that the requestor must fulfill via [`tasks/input_response`](#providing-input-for-tasks).
1. When a receiver receives a `tasks/result` request for a task in `working` status, it **MUST** block the response until the task reaches a terminal status or `input_required` status.
