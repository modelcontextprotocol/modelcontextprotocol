...
Before redirecting the user-agent, the client **MUST** record the `issuer` value from the selected authorization server's validated metadata document (see [Authorization Server Metadata Discovery](#authorization-server-metadata-discovery)) and associate it with the request's `state` value, alongside the PKCE code verifier. The validation in this section depends on that recorded value being authentic; it provides no protection if the expected issuer was obtained from an unvalidated source.

MCP authorization servers **SHOULD** include the `iss` parameter in authorization responses, including error responses, as defined in [RFC9207 Section 2](https://datatracker.ietf.org/doc/html/rfc9207#section-2), and advertise this by setting `authorization_response_iss_parameter_supported` to `true` in their metadata.
...
Comparing rather than discarding when `iss` is present without metadata advertisement is the local policy permitted by the carve-out in [RFC9207 Section 2.4](https://datatracker.ietf.org/doc/html/rfc9207#section-2.4) ("Local policy or configuration can determine whether to accept such responses, and specific guidance is out of scope"). It is sound here because the recorded issuer is always derived from a metadata document validated per [RFC8414 Section 3.3](https://datatracker.ietf.org/doc/html/rfc8414#section-3.3), so a present `iss` can be checked against an authentic baseline, and rejection on mismatch remains an unconditional **MUST**. This matches the unconditional client validation required by [FAPI 2.0 Security Profile Section 5.3.3.2](https://openid.net/specs/fapi-security-profile-2_0-final.html#section-5.3.3.2) and accommodates authorization servers that emit `iss` before updating their metadata.

A future revision of this specification is expected to upgrade authorization server inclusion of `iss` from **SHOULD** to **MUST**, at which point clients **MUST** reject any authorization response that lacks the parameter. Implementers are encouraged to emit and validate `iss` now to ease that transition.

If validation fails, the client **MUST NOT** transmit the authorization code to any token endpoint and **MUST** discard the per-request state for that attempt. Clients **MUST NOT** normalize issuer values (case, port, trailing slash, or percent-encoding) before comparison.
...
[Authorization Response Validation](#authorization-response-validation) mitigates this by binding the response to the authorization server the client recorded before redirecting, so the authorization code cannot be redeemed at an unintended token endpoint.
