...
Before redirecting the user-agent, the client **MUST** record the `issuer` value from the selected authorization server's metadata document ([RFC8414](https://datatracker.ietf.org/doc/html/rfc8414) or OpenID Connect Discovery) and associate it with the request's `state` value, alongside the PKCE code verifier.
...
Note: comparing (rather than rejecting) when `iss` is present without metadata advertisement is more lenient than [RFC9207 Section 2.4](https://datatracker.ietf.org/doc/html/rfc9207#section-2.4); this accommodates authorization servers that emit `iss` without yet advertising it in metadata.
