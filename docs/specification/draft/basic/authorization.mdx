...
Due to browser WebSocket API limitations that prevent setting headers during the WebSocket upgrade request, auth tokens are smuggled in using the WebSocket subprotocol list.

```javascript
const ws = new WebSocket('wss://mcp.example.com/mcp', ['mcp', `mcp.auth.${token}`]);
```
...
- Server **MUST** treat the `mcp.auth.${token}` the same way it would an `Authorization: Bearer ${token}` header in the Streamable HTTP protocol and validate it accordingly.
- Server **MUST** respond with the `mcp` as the chosen subprotocol, not `mcp.auth.${token}`.
