...
To mitigate this, MCP clients **SHOULD** validate the authorization response `iss` parameter against the issuer recorded before the authorization request, as described in [Authorization Response Validation](#authorization-response-validation). This binds the response to the authorization server the client actually selected and prevents the authorization code from being redeemed at an unintended token endpoint.
