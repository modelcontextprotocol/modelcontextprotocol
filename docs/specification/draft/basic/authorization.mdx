...
The responsibility for selecting which authorization server to use
lies with the MCP client, following the guidelines specified in
[RFC9728 Section 7.6 "Authorization Servers"](https://datatracker.ietf.org/doc/html/rfc9728#name-authorization-servers).

When multiple authorization servers are listed in `authorization_servers`, each is an
independent OAuth 2.0 authorization server whose client identifiers are unique to it
([RFC 6749 Section 2.2](https://datatracker.ietf.org/doc/html/rfc6749#section-2.2)). Clients **MUST** maintain
...
2. **Well-Known URI**: Serve metadata at a well-known URI constructed as specified in [RFC9728 Section 3](https://datatracker.ietf.org/doc/html/rfc9728#section-3). This can be either:
   - At the path of the server's MCP endpoint
   - At the root: `https://example.com/.well-known/oauth-protected-resource` (an MCP extension of RFC 9728 Section 3 for path-bearing endpoints)

MCP clients **MUST** support both discovery mechanisms and **MUST** be able to parse `WWW-Authenticate` headers on `HTTP 401 Unauthorized` responses, using the resource metadata URL from them when present; otherwise, they **MUST** fall back to constructing and requesting the well-known URIs in the order listed above.

MCP servers **SHOULD** include a `scope` parameter, as defined in
[RFC 6750 Section 3](https://datatracker.ietf.org/doc/html/rfc6750#section-3), in the
`WWW-Authenticate` header of `401 Unauthorized` responses. RFC 6750 makes this attribute
OPTIONAL; this specification raises its emission to **SHOULD**.
...
consistency in how they construct scope sets; [RFC 9728 Section 2](https://datatracker.ietf.org/doc/html/rfc9728#section-2)
permits `scopes_supported` to be non-exhaustive.
...
- The metadata document **MUST** include at least the following properties: `client_id`, `client_name`, `redirect_uris` (the Client ID Metadata Document specification requires only `client_id`; this specification additionally requires the other two)
...
MCP authorization servers **SHOULD** include the `iss` parameter in authorization responses, including error responses, as defined in [RFC9207 Section 2](https://datatracker.ietf.org/doc/html/rfc9207#section-2); RFC 9207 binds only servers that opt in, and this specification profiles support as a **SHOULD**. Authorization servers that include the `iss` parameter **MUST** advertise it in their metadata as required by [RFC9207 Section 2.3](https://datatracker.ietf.org/doc/html/rfc9207#section-2.3).
...
This validation applies equally to error responses ([RFC 9207 Section 2](https://datatracker.ietf.org/doc/html/rfc9207#section-2)); on mismatch the client **MUST NOT** act on or display `error`, `error_description`, or `error_uri`.

## Resource Parameter Implementation

MCP clients **MUST** implement Resource Indicators for OAuth 2.0 as defined in [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html).
RFC 8707 makes the `resource` parameter OPTIONAL and permits multiple values; this specification
tightens both: the parameter **MUST** be included in both authorization requests and token requests
([RFC 8707 Sections 2.1 and 2.2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2.1)), and its
single value **MUST** be the canonical URI of the MCP server that the client intends to use the
token with ([RFC 8707 Section 2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2)).
...
MCP clients **SHOULD** follow the guidance in [RFC 8707 Section 2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2) when selecting the resource value, including its recommendation to provide the most specific URI possible. While the canonical form uses lowercase scheme and host components, implementations **SHOULD** accept uppercase scheme and host components for robustness and interoperability.

[RFC 8707 Section 2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2) defines the requirements on the value, including that it be an absolute URI without a fragment component; a valid canonical URI for an MCP server is, for example, `https://mcp.example.com/mcp`.

> **Note:** While both `https://mcp.example.com/` (with trailing slash) and `https://mcp.example.com` (without trailing slash) are technically valid absolute URIs according to [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986), implementations **SHOULD** consistently use the form without the trailing slash for better interoperability unless the trailing slash is semantically significant for the specific resource.

MCP clients **MUST** send this parameter regardless of whether authorization servers support it; [RFC 8707 Sections 2.1 and 2.2](https://www.rfc-editor.org/rfc/rfc8707.html#section-2.1) show its use in both request types.
...
1. MCP clients **MUST** use the Authorization request header field defined in
   [OAuth 2.1 Section 5.1.1](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5.1.1);
   OAuth 2.1 states this method as a **SHOULD** and also allows the form-encoded body method of
   [OAuth 2.1 Section 5.1.2](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-5.1.2),
   while this specification requires the header method exclusively:
...
- **MUST** handle refresh tokens as specified in [OAuth 2.1 Section 4.3](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-4.3), including its confidentiality requirements, and **MUST NOT** assume refresh tokens will be issued; issuance remains at the authorization server's discretion per [OAuth 2.1 Section 3.2.3](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-3.2.3)
...
Servers **MUST** return the HTTP status codes defined in [RFC 6750 Section 3.1](https://datatracker.ietf.org/doc/html/rfc6750#section-3.1) for authorization errors: 400 for malformed authorization requests (`invalid_request`), 401 when authorization is required or the token is invalid (`invalid_token`), and 403 for invalid scopes or insufficient permissions (`insufficient_scope`). RFC 6750 states these mappings as per-code SHOULDs; this specification requires them.
...
scope during runtime operations, the server **SHOULD** respond with an
`insufficient_scope` challenge as defined in
[RFC 6750 Section 3.1](https://datatracker.ietf.org/doc/html/rfc6750#section-3.1), with two
additions to that specification:

- Servers **SHOULD** include the `scope` parameter, listing the scopes needed to satisfy the
  current operation (RFC 6750 makes this a MAY); servers are not required to include the
  client's previously granted scopes.
- Servers **SHOULD** include a `resource_metadata` parameter
  ([RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728)) carrying the URI of the Protected
  Resource Metadata document, for consistency with 401 responses.
...
[Resource Parameter Implementation](#resource-parameter-implementation) section;
