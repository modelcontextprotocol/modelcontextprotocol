...
- The metadata document **MUST** include at minimum: `client_id`, `client_name`, `redirect_uris`
- Clients **MUST** ensure the `client_id` value in the metadata matches the document URL exactly
- Clients **MAY** use `private_key_jwt` for token endpoint authentication with appropriate JWKS configuration
...
Authorization servers advertise support via their metadata:
```json
{
...
standardized way for clients to automatically register with new authorization servers.

Any authorization servers that support neither Client ID Metadata Documents nor Dynamic Client Registration
need to provide alternative ways to obtain a client ID (and, if applicable, client credentials). For these
authorization servers, MCP clients will have to either:
...
detailed in [draft-parecki-oauth-client-id-metadata-document-03 Section 7](https://www.ietf.org/archive/id/draft-parecki-oauth-client-id-metadata-document-03.txt).
Key considerations include:

#### SSRF Protection

Authorization servers fetching metadata documents **MUST** protect against Server-Side Request Forgery (SSRF) attacks:

- Validate URLs and resolved IP addresses before fetching
- Limit response size (recommended 5 kilobytes)
- Implement request timeouts
- Rate limit outbound metadata fetches
...
A malicious client could use this to send non-metadata requests on behalf of the authorization server,
such as requests to private administration endpoints the authorization server has access to.

#### DDoS Protection

Authorization servers **MUST** protect against being used as DDoS amplifiers:
...
The server will see the correct metadata document and the user will see the correct client name, making attack detection difficult.

Authorization servers:

- **SHOULD** display additional warnings for `localhost`-only redirect URIs
- **SHOULD** only fetch client metadata after authenticating the user
- **MAY** require additional attestation mechanisms for enhanced security
- **MUST** clearly display the client's hostname during authorization
...
- Display the client hostname prominently to prevent phishing
