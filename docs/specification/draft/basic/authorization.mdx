...
- Clients **MAY** use `private_key_jwt` for token endpoint authentication with appropriate JWKS configuration as described in [Section 6.2 of Client ID Metadata Document](https://www.ietf.org/archive/id/draft-ietf-oauth-client-id-metadata-document-00.html#section-6.2)
...
MCP clients **SHOULD** allow for support supplying static credentials such as those supplied by a preregistration flow. This could be:
