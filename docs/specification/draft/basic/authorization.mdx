...
This specification defines three OAuth 2.1 flows that implementations **SHOULD** support:
...
2. Client Credentials Flow - Used when a client needs to authenticate itself without user
   ...
   - Requires client authentication using either:
     - Asymmetric keys (JWT authentication) as defined in [RFC 7523 Section 2.2](https://datatracker.ietf.org/doc/html/rfc7523#section-2.2)
     - Client ID and secret

3. JWT Federation Flow - Used when a client leverages existing credentials from a federated
   identity provider. This flow:
   - Eliminates per-server credential management overhead
   - Uses JWT authorization grants as defined in [RFC 7523 Section 2.1](https://datatracker.ietf.org/doc/html/rfc7523#section-2.1)
   - Requires trust relationships between authorization servers and identity providers
...
- OAuth 2.0 Identity Chaining ([draft-ietf-oauth-identity-chaining](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-chaining/))
- WIMSE Headless JWT Authentication ([draft-levy-wimse-headless-jwt-authentication](https://www.ietf.org/archive/id/draft-levy-wimse-headless-jwt-authentication-01.html))
...
When supporting the JWT federation flow, Authorization Server metadata **MUST** include the following fields:

- `grant_types_supported`: **MUST** include `"urn:ietf:params:oauth:grant-type:jwt-bearer"`
- `token_endpoint_auth_methods_supported`: **MAY** include `"none"` to indicate support for unauthenticated clients using JWT assertions
...
- JWT Authentication (RECOMMENDED)
  - Clients use JWT Authentication as defined in [RFC 7523 Section 2.2](https://datatracker.ietf.org/doc/html/rfc7523#section-2.2).
- Client Secret
  - Clients use a Client Secret transmitted in the request content as defined in [OAuth 2.1 Section 2.4.1](https://www.ietf.org/archive/id/draft-ietf-oauth-v2-1-13.html#name-client-secret)
...
> **Note:** The `client_id` parameter is omitted from the request body as [RFC 7523 Section 3](https://datatracker.ietf.org/doc/html/rfc7523#section-3) specifies that client identification is conveyed through the `sub` claim within the JWT assertion.
...
The JWT Federation flow enables workload-to-workload authentication using JWT bearer tokens as authorization grants,
allowing clients to leverage existing trust relationships with identity providers when requesting access tokens from MCP servers.

#### When to Use JWT Federation

The Client Credentials flow requires MCP clients to establish direct trust relationships and manage distinct
credentials with each MCP server's authorization server. This approach creates operational overhead in multi-server environments.

JWT Federation simplifies this by allowing MCP clients to use JWT bearer tokens from trusted identity providers as
authorization grants. This enables clients to request access tokens from MCP servers based on existing trust relationships
between the server's authorization server and the identity provider that issued the JWT.

This pattern is utilized by specifications such as:

- [OAuth 2.0 Identity Chaining](https://datatracker.ietf.org/doc/draft-ietf-oauth-identity-chaining/)
- [WIMSE Headless JWT Authentication](https://www.ietf.org/archive/id/draft-levy-wimse-headless-jwt-authentication-01.html)

#### Authentication Method

Clients **MUST** use JWT Authorization Grants as defined in [RFC 7523 Section 2.1](https://datatracker.ietf.org/doc/html/rfc7523#section-2.1),
where the JWT assertion represents an identity from a federated identity provider.

#### Prerequisites

- The MCP server's authorization server **MUST** have a trust relationship with the identity provider that issued the JWT
- The authorization server **MUST** have an internal mechanism to map incoming federated JWTs to authorized principals in their system
- The JWT **MUST** contain appropriate claims to identify the requesting actor

#### Flow Steps

The complete JWT Federation flow proceeds as follows:
...
sequenceDiagram
    ...
    Note over C: Client already possesses<br/>JWT from Identity Provider
    ...
    Note over C: Present federated JWT assertion
    C->>A: POST /token with JWT assertion
    Note over A: Validate JWT signature<br/>via Identity Provider discovery<br/>Map federated identity to local principal
    A->>C: Access token
    C->>M: MCP request with access token
    M-->>C: MCP response
    Note over C,M: MCP communication continues with valid token
...
#### Non-normative HTTP Request Example
...
&assertion=eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJodHRwczovL2lkcC5leGFtcGxlLmNvbSIsInN1YiI6Indvcmtsb2FkLWlkZW50aWZpZXIiLCJhdWQiOiJodHRwczovL2F1dGguZXhhbXBsZS5jb20vdG9rZW4iLCJleHAiOjE3Mzg0NTAwMDAsImlhdCI6MTczODQ0NjQwMCwianRpIjoiYWJjMTIzIn0.signature
&resource=https%3A%2F%2Fmcp.example.com
```

> **Note:** Unlike the Client Credentials flow, no `client_id` is required as the client identity is derived from the federated JWT assertion claims as specified in [RFC 7523 Section 2.1](https://datatracker.ietf.org/doc/html/rfc7523#section-2.1).
