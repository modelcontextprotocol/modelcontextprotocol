...
1. **WWW-Authenticate Header**: Include the resource metadata URL in the `WWW-Authenticate` HTTP header when returning _401 Unauthorized_ responses, as described in [RFC9728 Section 5.1 "WWW-Authenticate Response"](https://datatracker.ietf.org/doc/html/rfc9728#name-www-authenticate-response).
...
MCP clients **MUST** support both discovery mechanisms and **MUST** be able to parse `WWW-Authenticate` headers when present and respond appropriately to `HTTP 401 Unauthorized` responses from the MCP server.
