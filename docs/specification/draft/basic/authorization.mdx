...
When available Dynamic Client Registration SHOULD be used in this flow:

- When using the JWT Authentication method as defined in [RFC 7523 Section 2.2](https://datatracker.ietf.org/doc/html/rfc7523#section-2.2) (RECOMMENDED), client MUST provide the jwks_uri (if it can be served) or Client's JSON Web Key in the jwks field as defined in [RFC 7591 Section 2](https://datatracker.ietf.org/doc/html/rfc7591#section-2).
- When using the Client Secret method, the server provides the client_id and client_secret in the response body as defined in [RFC 7591 Section 3.2.1](https://datatracker.ietf.org/doc/html/rfc7591#section-3.2.1).
