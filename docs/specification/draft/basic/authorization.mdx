...
- Sign-In with Ethereum ([EIP-4361](https://eips.ethereum.org/EIPS/eip-4361)) — optional, for SIWE-based authentication
...
When an MCP server supports SIWE and requires authentication, it **MAY** respond to unauthenticated requests with:
...
The `resource_metadata` URL points to a SIWE Protected Resource Metadata document. Metadata **MAY** also be served at the well-known path `/.well-known/siwe-protected-resource` (and path-scoped variants per [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728)).

### SIWE Metadata Parameters

The SIWE Protected Resource Metadata document includes:

- `resource` (REQUIRED): Canonical URI of the MCP server
- `siwe_nonce_endpoint` (REQUIRED): URL for obtaining a nonce
- `siwe_verify_endpoint` (REQUIRED): URL for verifying the SIWE signature and issuing a token
- `siwe_domain` (REQUIRED): Domain for the SIWE message
- `siwe_statement` (OPTIONAL): Statement shown to the user when signing
- `siwe_chain_id` (OPTIONAL): Chain ID (default 1)

### SIWE Flow Summary

1. Client receives 401 with `resource_metadata` pointing to SIWE metadata
2. Client fetches metadata and obtains a nonce from `siwe_nonce_endpoint`
3. Client constructs an [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) message and prompts the user to sign via wallet
4. Client POSTs `{ "message", "signature" }` to `siwe_verify_endpoint`
