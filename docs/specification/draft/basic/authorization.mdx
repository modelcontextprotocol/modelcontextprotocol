...
When multiple authorization servers are listed in
`authorization_servers`, clients **SHOULD** maintain separate
registration state (client credentials, tokens) per authorization
server. Authorization servers listed in `authorization_servers`
**MAY** be independent and do not necessarily share client
registrations or tokens. Clients **MUST NOT** assume that client
credentials valid for one authorization server will be accepted by
another.
...
Clients that persist client credentials obtained via Dynamic Client
Registration or pre-registration **MUST**, for each MCP server, associate those
...
from a different authorization server and **SHOULD** re-register
