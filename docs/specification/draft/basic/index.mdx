...
- `mimeType`: Optional MIME type (e.g., "image/png", "image/svg+xml") if the server's type is missing or generic
- `sizes`: Optional size specification (e.g., "48x48", "any" for scalable formats like SVG, or "48x48 96x96" for multiple sizes)

**Security considerations:**

Consumers SHOULD take appropriate security precautions when handling icons:

- Verify that icon URLs are from the same domain as the server or a trusted domain if possible. This minimizes the risk of exposing data or tracking information to third-parties.
- Be cautious when consuming SVG files as they can contain executable JavaScript.
  - Disallow SVG files or sanetize SVG files before rendering.
