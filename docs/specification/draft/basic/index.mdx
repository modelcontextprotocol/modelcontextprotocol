...
[Success responses](/specification/draft/schema#jsonrpcresponsesuccess) are sent when the operation completes successfully.
...
- Success responses **MUST** include the same ID as the request they correspond to.
- Success responses **MUST** include a `result` field.
- The `result` **MAY** follow any JSON object structure.

#### Error Responses

[Error responses](/specification/draft/schema#jsonrpcresponseerror) are sent when the operation fails or encounters an error.
