...
MCP provides a [Payment](/specification/draft/basic/payment) framework for premium features and usage-based billing using the X402 protocol.
Implementations using an HTTP-based transport **MAY** implement payment handling for tool invocations,
whereas implementations using STDIO transport **SHOULD NOT** follow this specification.

The payment specification enables servers to request payment for specific tools or resources while maintaining security through X402-compliant facilitators.
