...
[Responses](/specification/draft/schema#jsonrpcresponse) are sent in reply to requests, containing the result or error of the operation.
...
- Responses **MUST** include the same ID as the request they correspond to (except in error cases where the ID could not be read due a malformed request).
