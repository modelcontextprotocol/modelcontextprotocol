...
- **Transports**: Standard mechanisms for client-server communication (stdio and Streamable HTTP)
- **Authorization**: Authentication and authorization framework for HTTP-based transports
- **Server Features**: Prompts, resources, and tools exposed by servers
- **Client Features**: User input elicitation provided by clients
- **Utilities**: Cross-cutting concerns like cancellation, progress tracking, and argument completion

All implementations must support the base protocol, lifecycle management, and at least
one transport. Other components may be implemented based on the specific needs of the
application.

These components establish clear separation of concerns while enabling rich
...
- A `resultType` of `"input_required"` indicates more information is needed to process the request. The result contains an
  [`InputRequiredResult`](/specification/draft/schema#inputrequiredresult) describing the requested input; see
  [Multi Round-Trip Requests (MRTR)](/specification/draft/basic/utilities/mrtr) for how the client gathers it and retries the request.
