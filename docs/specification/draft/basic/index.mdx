...
- Error codes in the range `-32768` to `-32000` (inclusive) are reserved by JSON-RPC and MCP. Within that range, codes `-32000` to `-32099` are **reserved for MCP-defined errors**; implementations **MUST NOT** use codes in this range for their own application-specific errors.
- Implementations **MAY** define application-specific error codes outside the range `-32768` to `-32000`.
