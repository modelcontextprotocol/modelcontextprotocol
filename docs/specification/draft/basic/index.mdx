...
- **`-32000` to `-32009` — implementation-defined.** Existing SDKs and
  implementations use codes in this sub-range for their own purposes (for
  example, local timeout or connection errors). The MCP specification will
  never define error codes in this sub-range, and new implementation-defined
  codes **SHOULD NOT** be introduced here. With the exception of the historical
  codes listed below, receivers **MUST NOT** assign cross-implementation
  semantics to these codes: the same code may carry different meanings in
  different implementations.
- **`-32010` to `-32099` — reserved for the MCP specification.** Error codes
...
Error codes defined by this version of the specification:
...
New codes are allocated sequentially starting at `-32020` and proceeding
toward `-32099`. Codes `-32010` to `-32019` are intentionally left unallocated
...
`-32602`, though [clients **SHOULD** still accept
`-32002`](/specification/draft/server/resources#error-handling) from servers
implementing earlier versions) and `-32042` (URL elicitation required,
2025-11-25 only).
