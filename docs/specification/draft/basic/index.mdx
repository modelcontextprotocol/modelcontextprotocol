...
MCP provides a [Payment](/specification/draft/basic/payment) framework enabling servers to monetize their tools through various payment methods.

The payment specification is protocol-agnostic and supports multiple payment protocols (X402 for blockchain, traditional processors for fiat). It works with any transport (HTTP, STDIO, SSE) by including payment information in the `payment` field of tool call parameters.

Servers can advertise payment requirements in tool definitions, and clients handle payment through protocol-specific payment providers. The framework includes capability declaration, payment discovery, standardized payment challenges (error `-32402`), and payment verification flows.
