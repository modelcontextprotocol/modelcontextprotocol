...
[subscriptions-listen]: /specification/draft/server/resources#subscriptions-listen
...
any other request. If the server returns `Method not found` or
`UnsupportedProtocolVersionError`, the client falls back to the legacy
...
This replaces the [HTTP+SSE transport][http-sse] from
...
- The server **MUST NOT** send independent JSON-RPC _requests_ on this stream.
  ...
  [`IncompleteResult`][incomplete-result] per
...
[tasks primitive][tasks]; resumable SSE
