...
When using transports that have their own timeouts, implementations **SHOULD** configure transport timeouts based on the timeout declarations negotiated during initialization. For example, HTTP client timeouts **SHOULD** be set slightly higher than the declared operation timeout to account for network latency.
