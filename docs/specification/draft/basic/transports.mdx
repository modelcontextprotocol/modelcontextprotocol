...
This section defines bidirectional transport of MCP over a WebSocket connection which provides better support for maintaining long-lived connections particularly in cloud environments and through intermediaries. Long-lived connections are necessary for server-initiated notifications like `notifications/resources/updated` and `notifications/*/list_changed` which enable real-time use cases.

The server **MUST** provide an HTTP endpoint path that supports WebSocket upgrade HTTP request. This **SHOULD** be the same endpoint as the Streamable HTTP transport.
...
A key motivation for this WebSocket transport is to enable bi-directional usage including server-initiated notifications, which is unworkable without session state that survives connection disruptions. Therefore, servers and clients using this WebSocket transport **MUST** provide MCP session support.

1. A server using the WebSocket transport **MUST** assign an MCP session ID in response 
   to an `initialize` call, by including a `result.mcpSessionId` field in the JSON-RPC response.
   - The session ID **SHOULD** be globally unique and cryptographically secure (e.g., a
     securely generated UUID, a JWT, or a cryptographic hash).
   - The session ID **MUST** only contain visible ASCII characters (ranging from 0x21 to
     0x7E).
2. Clients using the WebSocket transport **MUST** include the `params.mcpSessionId` field
   in all of their subsequent MCP communications.
   - Servers **MUST** respond to requests without a `params.mcpSessionId` field (other than initialization) with a JSON-RPC error code `-32602` and message "Missing mcpSessionId".
3. The server **MAY** terminate the session at any time, after which it **MUST** respond
   to requests containing that `mcpSessionId` with a JSON-RPC error code `-32602` and message "Invalid mcpSessionId".
4. The server **MUST** include a `params.mcpSessionId` field in all of its MCP 
   communications.
5. The relationship of MCP sessions to WebSocket connections is many to one, so:
...
6. Subscriptions (e.g. `resources/subscribe`, `list_changed`, etc.) are associated with a
   particular session so server-initiated notifications **MUST** only be sent over the active connection for that session.
7. Clients **MAY** renegotiate version and capabilities for a given session at any time
   by sending a new `initialize` message with the desired parameters and including `params.mcpSessionId`.
...
- If auth fails, reject the upgrade request
- If auth succeeds, accept the WebSocket connection
- Expect session to be established before processing any messages

#### Normal Operation

During normal operation:
- Client **MUST** include the `params.mcpSessionId` in all messages except the first `initialize`
...
To reconnect:
- Client **MAY** include one `mcp.session-id.${mcpSessionId}` value in the WebSocket subprotocols list via the `Sec-WebSocket-Protocol` header in the WebSocket upgrade request to help with intermediary affinity
...
- Client **MUST** include the `mcpSessionId` from the previous session in any message after reconnection
- Client **MAY** send an `initialize` message including a `params.mcpSessionId` after reconnecting over a different WebSocket connection to an existing session
  - Server **MUST** associate this new connection with that session and update any renegotiated version or capabilities
- Regardless, upon receipt of the first message for an existing session over a different connection, the server **MUST** associate this new connection with previously established session state making this the only active connection for the session
- Server **MUST** disassociate any previous connection associated with the same `mcpSessionId` and ignore any messages that arrive for the MCP session on any connection but the active one
...
- Clients and servers **SHOULD** keep track of unacknowledged requests and resend them if necessary after reconnection
