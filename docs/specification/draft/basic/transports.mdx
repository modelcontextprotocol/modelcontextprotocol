...
For task-related operations (`tasks/get` and `tasks/cancel`), the `Mcp-Name` header **MUST** be set to the value of `params.taskId`. This enables infrastructure to route task requests to the correct server instance, as tasks often involve server-side state that must be consistently co-located.
