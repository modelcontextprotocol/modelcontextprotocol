...
4. If the server cannot accept the input, it **MUST** return an HTTP error status code
   (e.g., 400 Bad Request).
   - If the input is not accepted due to unmet `Accept` header requirements, the error
     **MUST** be of HTTP status code 406 Not Acceptable.
   - If the input is not accepted due to unmet body requirements, the error
