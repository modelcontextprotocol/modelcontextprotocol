...
Under certain conditions, attackers can exploit the MCP server-managed consent cookie to redirect authorization codes to malicious clients without re-prompting the user.

These conditions include:

- MCP server uses a **static client ID** with a third-party authorization server
- MCP server allows MCP clients to **dynamically register** (each getting their own client_id)
...
- Validate at the callback endpoint that the `state` query parameter exactly matches the stored value
