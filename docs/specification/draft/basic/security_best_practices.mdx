...
When XSS vulnerabilities are combined with `stdio` transport capabilities, attackers can escalate web-based attacks to full system compromise. See [stdio Transport Security](#stdio-transport-security) for detailed attack vectors and mitigations.
...
sequenceDiagram
    ...
    Attacker->>Client: Malicious authorization URL (javascript:)
    ...
    Client-->>Attacker: Exfiltrate data/establish persistence
```
...
The `stdio` transport, when combined with other vulnerabilities, can provide a critical escalation path from web-based attacks to full system compromise.

#### Attack Description

MCP clients that support the `stdio` transport typically run a local proxy service that can spawn MCP servers as child processes. This architecture creates a privileged escalation path when combined with client-side vulnerabilities:

1. Attacker achieves XSS or other client-side code execution (e.g., through OAuth URL vulnerabilities)
2. Malicious JavaScript accesses the MCP proxy authentication token from the client's environment
3. JavaScript makes authenticated requests to the local MCP proxy service
4. Proxy spawns arbitrary commands via the `stdio` transport
5. Attacker achieves Remote Code Execution with user privileges

#### Risks

- **Privilege Escalation**. Web-based vulnerabilities (XSS) can escalate to arbitrary code execution on the host system
- **Authentication Bypass**. Stolen proxy authentication tokens allow unauthorized access to `stdio` transport capabilities
...
MCP proxy services **MUST** implement robust authentication mechanisms:

- **MUST** use cryptographically secure, unpredictable authentication tokens
- **SHOULD** rotate authentication tokens regularly or on each session
- **MUST** validate that all `stdio` requests are properly authenticated
- **SHOULD** implement request rate limiting to prevent abuse

**stdio Transport Restrictions**

MCP proxy services **SHOULD** implement additional security controls for `stdio` transport:

- Maintain an allowlist of permitted MCP server executables
...
MCP clients **MUST** protect proxy authentication tokens:

- Store authentication tokens securely (not in accessible JavaScript globals)
- Use secure communication channels between client and proxy
- Implement token scoping to limit what operations can be performed
- Clear authentication tokens when clients disconnect or terminate
