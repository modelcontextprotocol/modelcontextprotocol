...
Generated session IDs (e.g., UUIDs) **SHOULD** use secure random number generators with at least 128 bits of entropy. Avoid predictable or sequential session identifiers that could be guessed by an attacker. Rotating or expiring session IDs can also reduce the risk.
