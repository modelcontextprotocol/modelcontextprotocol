...
```json
{
  ...
  "result": {
    ...
    "mcpSessionId": "abc123"  // Required for WebSocket transport, optional otherwise
  }
}
```
...
```
<Note>
The `mcpSessionId` field is required when using the WebSocket transport and optional for other transports. For more details, see [the MCP Session section in Transports](/specification/draft/basic/transports#mcp-session-management-for-websockets).
</Note>
...
server. For details, see [the Protocol Version Header section in Transports](/specification/draft/basic/transports#protocol-version-header).

Similarly, if using WebSocket transport, the client **MUST** include an `mcp.version.${protocolVersion}` value in the WebSocket subprotocols list via the `Sec-WebSocket-Protocol` header in the WebSocket upgrade request.
</Note>
