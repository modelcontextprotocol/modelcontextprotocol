...
- Optionally, the set of protocol versions supported
- Client capabilities
...
In the `initialize` request, the client **MUST** send a preferred protocol version it supports.
This **SHOULD** be the _latest_ version supported by the client.
The client **MAY** also send a set of supported protocol versions.

If the server supports the preferred protocol version, it **MUST** respond with the same
version. If the client supplied additional supported protocol versions, the server **MUST**
respond with one of the protocol versions supported by the client. Otherwise, the server
**MUST** respond with another protocol version it supports. This **SHOULD** be the _latest_
