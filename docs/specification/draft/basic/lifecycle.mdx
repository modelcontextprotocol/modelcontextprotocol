...
any other requests to learn the server's supported versions up front. This is
optional — a client is free to invoke any RPC inline and handle
...
- **STDIO.** Because there is no per-request status code to drive fallback,
  ...
  [`server/discover`](/specification/draft/schema#discoverrequest) first,
  setting its preferred modern version in `_meta`. If the server returns
  `Method not found` or `UnsupportedProtocolVersionError`, fall back to the
