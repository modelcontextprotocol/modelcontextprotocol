...
All server-to-client messages arrive on `stdout`, one message per line,
multiplexed onto a single shared channel: responses to client requests,
in-flight notifications (`notifications/progress`,
`notifications/message`), and deliveries on a
[`subscriptions/listen`][subscriptions-listen] stream. The server **MUST
NOT** write JSON-RPC _requests_ to `stdout`.
