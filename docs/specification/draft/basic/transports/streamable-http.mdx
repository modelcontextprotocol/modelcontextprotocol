...
4. The body of the HTTP POST **MUST** be a single JSON-RPC _request_ or
   _notification_. Clients never send JSON-RPC _responses_, because servers
   do not initiate requests: server-to-client interactions are embedded in
   results per [MRTR][mrtr] (see [Receiving Messages](#receiving-messages)).
...
See [Lifecycle: Backward Compatibility][lifecycle-compat] for details and a
