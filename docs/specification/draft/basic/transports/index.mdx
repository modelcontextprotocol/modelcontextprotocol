...
This page defines the transport-independent model for MCP communication: the
message flow shared by every transport, how request metadata and
cancellation map onto a transport, and the requirements for defining new
transports.
...
does not define what the messages mean. The binding pages specify the
standard transports:
