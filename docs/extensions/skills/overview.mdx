...
This example shows successful loading of a skill with a file manifest, after
...
sequenceDiagram
    ...
    opt Entry not already available
        Host->>Server: skills/get (URI)
        Server-->>Host: Skill entry with frontmatter and manifest
    end
    Note over Host: Retain the entry with its server identity and URI
...
A known skill URI can come from the user, another skill, or server instructions;
listing is not a prerequisite. When a listing already supplied the entry,
`skills/get` is unnecessary. All skill reads use the originating server.

If approval is denied or verification fails, the host does not load or use the
content. Recovering from a changed manifest requires refreshing the entry and
obtaining any required approval again, as described below.
