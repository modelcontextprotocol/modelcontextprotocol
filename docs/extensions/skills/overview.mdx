...
Host applications determine which skills are available to the model or user and
when their instructions are loaded into model context. Skill metadata supports
discovery without retrieving file contents. Instructions and supporting files
are retrieved as needed.

Reading `SKILL.md` through `resources/read` does not itself activate a skill.
Activation occurs through the host's skill-loading path, which verifies the
content and applies any required user approval before loading it into model
context.
...
<Note>
  ...
  [request metadata](/specification/draft/basic/index#meta).
</Note>
