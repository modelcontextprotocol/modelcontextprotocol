...
## Example subregistries

- **[RAGMap](https://ragmap-api.web.app)** — RAG-focused subregistry: consumes the official registry, enriches with relevance scoring and filters (e.g. retrieval, transport). Provides a registry-compatible API plus semantic search and [MCP tools](https://ragmap-api.web.app/.well-known/agent.json) for discovery.
