...
For servers with a `com.example/*` name, the `url` property of the `remotes` entry must be on the `example.com` domain or one of its subdomains.

```json server.json highlight={3,10}
{
  "$schema": "https://static.modelcontextprotocol.io/schemas/2025-10-17/server.schema.json",
  "name": "com.example/acme-analytics",
  "title": "ACME Analytics",
...
For servers with a `io.github.username/*` name, there are no URL restrictions.
