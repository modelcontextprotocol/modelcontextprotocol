...
Remote servers should use the Streamable HTTP transport. The SSE transport is [deprecated](/specification/draft/deprecated) in the specification and scheduled for removal, so publish an `"sse"` remote only to support existing clients. Remote servers can also support both transports simultaneously at different URLs.
