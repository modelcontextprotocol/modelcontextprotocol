...
In addition, the [Files WG](/community/working-groups/files), which consolidates the former File Uploads and Filesystems WGs, continues on file transfer, resource write operations, and filesystem-like resource semantics (range reads, hierarchical listing).
