...
<McpClient
  ...
  supports="Resources, Prompts, Tools, Discovery, Instructions, Tasks, DCR"
>

`mcpc` is a universal CLI client for MCP — proof that CLI and MCP are complementary, not competing. Most agents misuse MCP by dumping all tool definitions and results into context, wasting tokens and causing context rot. `mcpc` solves this by giving agents full MCP protocol access through a single `Bash()` tool call, with self-documenting commands agents pick up via `--help` without external skills. MCP handles the remote concerns (server discovery, auth, payments, access control); the shell provides the local interface agents already know.

**Key features:**

- **Code mode in the shell:** `--json` output composes with `jq`, `xargs`, and shell pipelines, letting agents write MCP workflows as shell scripts — often more accurate and token-efficient than naive tool calling. `--schema` validates tool schemas against snapshots to catch breaking changes.
- **Progressive tool discovery:** `grep` searches tools, resources, and prompts across all active sessions with regex, so agents load only what they need instead of injecting every tool definition into the prompt.
