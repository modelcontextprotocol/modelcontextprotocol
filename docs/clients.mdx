...
| [Alice][Alice]                              | ✅          | ✅        | ✅      | ❓                     | ✅          | ❌    | Supports tools, resources, prompts, and sampling with stdio, streamable HTTP, and SSE transport.    |
...
### Alice
[Alice](https://heyalice.app) is a desktop application for macOS, Windows, and Linux. It connects directly to AI providers with configuration and chat history stored locally. Alice's servers only retain license information and basic usage statistics.

**Key features:**
- Full support for MCP resources, prompts, tools, and sampling  
- Multiple transport protocols: stdio, streamable HTTP, and SSE  
- Integration with OpenAI, Anthropic, Gemini, xAI, Groq, Ollama, OpenRouter, and custom servers  
- Assistant profiles with custom settings, prompts, and skills  
- Skill/Prompt Library with configurable prompts that can be called during chat sessions  
- Documents that give assistants access to text-based files in an exploratory, agentic way  
- Assistant library available through subscription plans  
- Custom connections for direct integration with local or remote servers supporting the OpenAI API format
