...
stdio Bus is an MCP client runtime. It connects to MCP servers, executes client operations (`initialize`, `tools/list`, `tools/call`, `resources/read`, prompts, sampling), and manages sessions across multiple transports (stdio, Unix socket, TCP, shared memory). Available as SDKs for [Rust](https://crates.io/crates/stdiobus-client), [Node.js](https://www.npmjs.com/package/@stdiobus/node), [Python](https://pypi.org/project/stdiobus/), C++, and Go.

**Key features:**

- Connect to MCP servers and perform full client operations including tools, resources, prompts, sampling, and discovery notifications.
- Run multiple MCP server workers with automatic health monitoring, crash recovery, and session-based routing.
- Choose your transport: stdio, Unix socket, TCP, POSIX shared memory, or POSIX message queues.
- Embed into any application via SDKs or deploy standalone with official [Docker images](https://hub.docker.com/r/stdiobus/stdiobus).
- Launch agents dynamically from the ACP Registry with support for binary, npx, and uvx distributions.
