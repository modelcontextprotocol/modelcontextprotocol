...
`mcpc` is a universal CLI client for MCP — a Swiss Army knife that maps every MCP operation to intuitive shell commands for interactive use, scripting, and AI coding agents. It works with any MCP server over Streamable HTTP or stdio, with or without a config file.
...
- **Code mode and scripting:** `--json` output composes with `jq`, `xargs`, and shell pipelines; `--schema` validates tool schemas against snapshots to catch breaking changes.
- **Cross-session discovery:** `grep` searches tools, resources, and prompts across all active sessions with regex support.
...
- **Lightweight and cross-platform:** minimal dependencies, runs on macOS, Windows, and Linux. Install via `npm install -g @apify/mcpc`.
