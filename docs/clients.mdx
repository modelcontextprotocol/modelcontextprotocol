...
- Swiss knife for MCP: supports stdio and streamable HTTP, server config files and zero config, OAuth 2.1, HTTP headers, and main MCP features.
...
- JSON output and schema validation allow stable integration with other CLI tools, scripting, and MCP **code mode** in shell.
