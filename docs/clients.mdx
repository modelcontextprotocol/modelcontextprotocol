...
`mcpc` is a universal CLI client for MCP — a Swiss Army knife that brings MCP to the shell. AI coding agents only need `Bash()` to connect to any MCP server, discover tools progressively, and compose multi-step workflows as shell scripts. Works with any server over Streamable HTTP or stdio, with or without a config file, and requires no LLM of its own.
...
- **Progressive tool discovery:** `grep` searches tools, resources, and prompts across all active sessions with regex, so agents load only what they need instead of stuffing every tool definition into context.
...
- **Lightweight and cross-platform:** no LLM required, minimal dependencies, runs on macOS, Windows, and Linux. Install via `npm install -g @apify/mcpc`.
