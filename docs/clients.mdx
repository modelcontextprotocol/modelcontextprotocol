...
<McpClient
  ...
  sourceCode="https://github.com/stdiobus/stdiobus"
  supports="Resources, Prompts, Tools, Discovery, Instructions, Sampling, Roots, Elicitation, Tasks, OAuth Client Credentials"
  instructions="https://stdiobus.com/getting-started"
>

stdio Bus is an MCP client runtime that combines multi-language SDKs with a supervised process runtime. It keeps agent and server processes healthy, preserves session continuity, and routes messages reliably across long-running workloads.

**Key features:**

- Five transport modes: stdio, Unix socket, TCP, shared memory, and message queues — use what fits your environment.
- Pluggable framing and protocol options including text and binary, with automatic translation between modes.
- Built-in process supervision with restart policies, health monitoring, and flow control for high-throughput traffic.
- Session-aware routing that preserves conversation continuity across worker pools with reliable request/response matching.
- Connect to MCP servers from [Rust](https://crates.io/crates/stdiobus-client), [Node.js](https://www.npmjs.com/package/@stdiobus/node), [Python](https://pypi.org/project/stdiobus/), C++, and Go using dedicated SDKs.
- Deploy standalone with official [Docker images](https://hub.docker.com/r/stdiobus/stdiobus) or embed directly into your application via the C API.

**Learn more:**

- [stdio Bus – Agentic Orchestration Ecosystem](https://stdiobus.com)
