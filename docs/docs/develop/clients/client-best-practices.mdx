...
**Step 1: Generate a programmatic API from MCP schemas.** The client reads each server's tool definitions and produces typed functions based on each tool's `outputSchema`:
