...
In the root of the server folder create two files: `Program.cs`, `ProtectedMcpServer.csproj`, and one folder - `Tools`. Fill `Program.cs` with:
...
In the `Tools` folder, create `MathTools.cs` and fill with:
...
Then from the serve's root, run:
