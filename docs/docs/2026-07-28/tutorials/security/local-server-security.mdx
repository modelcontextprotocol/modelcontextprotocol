...
[Authorization](/docs/2026-07-28/tutorials/security/authorization) and
...
to enforce. This is by design: the SDKs assume full trust between stdio
peers, and any isolation or security fencing around a local server is the
responsibility of the host application and of whoever operates the machine.
...
- **Grant the narrowest root that works.** Configure filesystem-oriented servers with a specific project directory, passed explicitly through the server's configuration or tool parameters — never your home directory.
