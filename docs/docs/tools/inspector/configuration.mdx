...
The whole scripting surface belongs to the CLI. See [CLI client](/docs/tools/inspector/cli) for usage.
...
You do not have to hand-write these — the web client can [import an existing client config](/docs/tools/inspector/recipes#importing-an-existing-client-config) from Claude Desktop, Cursor, Cline, or VS Code, or a registry `server.json`.
