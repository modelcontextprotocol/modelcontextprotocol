...
[removes protocol-level sessions and the `Mcp-Session-Id` header](/specification/draft/changelog)
from the Streamable HTTP transport. Servers implementing the current
revision do not mint session IDs, so the attacks below apply to servers
implementing protocol versions `2025-11-25` and earlier. Servers that
need cross-call state under the current revision instead mint explicit
state handles and receive them back as ordinary tool arguments. See
[Securing State Handles](#securing-state-handles) below.
...
using protocol versions `2025-11-25` and earlier, the following attack
...
#### Securing State Handles

Under protocol revision `2026-07-28`, the protocol is
[stateless](/specification/draft/basic/index#statelessness): servers
that need state spanning multiple requests mint an explicit handle
(for example a shopping cart ID or a workflow ID) and receive it back
as an ordinary tool argument on each request. A guessed or stolen
handle raises the same impersonation risk as a hijacked session ID, so
...
#### Mitigation for Session-Based Versions

Servers that still implement protocol versions `2025-11-25` and
earlier should implement the following mitigations to prevent session
hijacking and event injection attacks:
