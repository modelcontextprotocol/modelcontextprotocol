...
With all the metadata out of the way, the client now needs to make sure that it's registered with the authorization server. This can be done in a few ways.

First, the client can be **pre-registered** with a given authorization server, in which case it can have embedded client registration information that it uses to complete the authorization flow. This could be:
...
Second, the client can use a **Client ID Metadata Document** (CIMD), where the client uses an HTTPS URL that it controls as its `client_id` and hosts its own metadata at that URL for the authorization server to fetch. A metadata document looks like this:
