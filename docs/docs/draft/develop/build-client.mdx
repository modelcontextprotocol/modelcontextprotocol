...
This provides similar functionality but uses a WebFlux-based SSE transport implementation. The SSE transport is [deprecated](/specification/draft/deprecated), so prefer Streamable HTTP for new production deployments.
