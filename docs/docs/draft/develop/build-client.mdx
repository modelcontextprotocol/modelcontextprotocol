...
This provides similar functionality but uses a WebFlux-based SSE transport implementation. The SSE transport is [deprecated](/specification/draft/deprecated) in the specification, so prefer Streamable HTTP for new production deployments.
