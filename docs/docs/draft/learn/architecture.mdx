...
The `tools/list` request carries no parameters beyond the standard `_meta` fields that accompany every MCP request.
...
When the server's available tools change—such as when new functionality becomes available, existing tools are modified, or tools become temporarily unavailable—the server delivers a notification on that stream:
