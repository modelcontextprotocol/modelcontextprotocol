...
The `tools/list` request carries only the standard `_meta` fields that identify the client on every request, with no tool-specific parameters.
...
The result is marked `"resultType": "complete"` and carries two caching fields that complete list and read results must include:

- **`ttlMs`**: A freshness hint in milliseconds. The client may treat the response as fresh for this long before re-fetching. Here `300000` means the tool list can be cached for five minutes.
- **`cacheScope`**: Indicates who may cache the response. `"public"` responses contain no user-specific data and may be served from shared caches. `"private"` responses may only be reused within the same authorization context.
...
#### Opting In with `subscriptions/listen`

Change notifications are opt-in. To receive them, the client opens a long-lived notification stream by sending a `subscriptions/listen` request with a `notifications` filter naming the event types it wants. Here the client asks for tool list changes:
...
Every client request carries the `io.modelcontextprotocol/protocolVersion`, `clientInfo`, and `clientCapabilities` fields in `_meta`, so the server can identify the client without relying on connection state.

The server acknowledges the filter with `notifications/subscriptions/acknowledged` as the first message on the stream, then delivers matching notifications as changes occur. The server never sends notification types the client did not request.

#### Understanding Tool List Change Notifications

When the server's available tools change—such as when new functionality becomes available, existing tools are modified, or tools become temporarily unavailable—the server notifies clients that opted in:
...
2. **Opt-In Based**: This notification is only sent to clients that requested `"toolsListChanged": true` in their `subscriptions/listen` filter.
