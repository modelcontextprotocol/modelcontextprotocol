...
The server acknowledges the filter with `notifications/subscriptions/acknowledged`, which is the first message carrying that subscription's ID in `_meta`, then delivers matching notifications as changes occur. The server never sends notification types the client did not request.

#### Understanding Tool List Change Notifications

When the server's available tools change (for example, when new functionality becomes available, existing tools are modified, or tools become temporarily unavailable), the server notifies clients that opted in:
