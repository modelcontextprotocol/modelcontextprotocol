...
<Warning>
  The Roots feature is [deprecated](/specification/draft/deprecated) as of
  protocol version `2026-07-28` and is scheduled for removal in a future
  revision. New implementations should not adopt it. Instead, pass directories
  or files via tool parameters, resource URIs, or server configuration.
</Warning>
...
<Warning>
  The Sampling feature is [deprecated](/specification/draft/deprecated) as of
  protocol version `2026-07-28` and is scheduled for removal in a future
  revision. New implementations should not adopt it. Instead, integrate directly
...
Like elicitation, sampling follows the [Multi Round-Trip Requests](/specification/draft/basic/patterns/mrtr) pattern. When a server needs a completion while processing a request such as `tools/call`, it responds with an `InputRequiredResult` whose `inputRequests` field carries a `sampling/createMessage` request. The client obtains the generation and retries the original request with the response included in `inputResponses`.
...
**Request parameters example (the `params` of a `sampling/createMessage` request delivered inside `InputRequiredResult.inputRequests`):**
