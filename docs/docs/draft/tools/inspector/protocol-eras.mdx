...
Era selection works the same way in all three clients — the same setting, the same negotiation, the same result.
...
Both eras reject with `-32602`; what changes is how the Inspector presents it. On a legacy connection you get one generic JSON-RPC failure and have to read the message to tell which case you hit. On a modern connection the Inspector classifies it first, so "the server doesn't have that tool" is visually distinct from "your arguments were wrong."
