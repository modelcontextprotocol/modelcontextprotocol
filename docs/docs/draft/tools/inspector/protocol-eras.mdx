...
This page explains the setting, how negotiation works, and then walks the two eras feature by feature.
...
Every section below ends with a **Reproduce with …** pointer to a JSON config. Those configs come from the **composable test servers** shipped in the Inspector repository — ready-made reproductions of each behavior. Clone the repo, build the test servers, then point the Inspector at the config the section names.
...
<Note>
  ...
  server→client requests aren't allowed there. MRTR is its modern replacement.
</Note>
