...
`--transport` accepts `http` (Streamable HTTP) and `sse`. If the server is protected, see [Authorization](/docs/draft/tools/inspector/authorization) — you don't need to do anything special up front; the Inspector reacts to the `401`.
