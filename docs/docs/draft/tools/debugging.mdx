...
2. **Server logging**: structured logs to stderr (stdio transport) or via
   [OpenTelemetry](https://opentelemetry.io/) (all transports). The protocol's
   [logging](/specification/draft/server/utilities/logging) feature
   (`notifications/message`) is deprecated as of protocol version `2026-07-28`
   but remains available during the deprecation window.
...
or [OpenTelemetry](https://opentelemetry.io/), or standard HTTP tooling (curl,
browser DevTools Network panel) to inspect requests and SSE streams.

<Warning>

The protocol's [logging](/specification/draft/server/utilities/logging)
feature is deprecated as of protocol version `2026-07-28`. It remains available
during the deprecation window, but new servers should log to stderr (stdio
transport) or use [OpenTelemetry](https://opentelemetry.io/) instead.
