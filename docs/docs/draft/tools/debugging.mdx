...
2. **Server logging**: structured logs to stderr (stdio transport) or via
   [`notifications/message`](/specification/draft/server/utilities/logging#log-message-notifications)
   (all transports).
...
DevTools Network panel) to inspect requests, the
[request metadata headers](/specification/draft/basic/transports/streamable-http#request-metadata)
(`MCP-Protocol-Version`, `Mcp-Method`, `Mcp-Name`), and SSE streams.

<Warning>

The [Logging feature](/specification/draft/server/utilities/logging)
(`notifications/message` and the per-request
`io.modelcontextprotocol/logLevel` field) is deprecated as of protocol version
`2026-07-28`. New implementations should not adopt it. Prefer logging to
stderr on stdio transports, or [OpenTelemetry](https://opentelemetry.io/) for
structured observability.

</Warning>

For all [transports](/specification/draft/basic/transports), you can also
...
MCP defines eight
[RFC 5424 severity levels](/specification/draft/server/utilities/logging#log-levels)
(`debug` through `emergency`). Clients set the minimum level per request via
the
[`io.modelcontextprotocol/logLevel`](/specification/draft/server/utilities/logging#requesting-log-messages)
field in `_meta`. Servers only emit `notifications/message` for requests that
include it.
...
5. Check the
   ...
   `io.modelcontextprotocol/clientCapabilities`. A request missing any of
   these is rejected with error `-32602` (Invalid params), the same code
   returned for many other malformed inputs. If the server needs a capability
   the request's `clientCapabilities` did not declare, such as
