...
<Note>
  Note that local MCP servers using the STDIO transport typically serve a single
  MCP client, whereas remotely hosted MCP servers using StreamableHttp may
  accept concurrent connections from many MCP clients.
</Note>
