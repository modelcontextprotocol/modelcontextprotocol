...
<Note>
  This does not imply that an MCP server itself can only serve a single client — an MCP server **may accept connections from multiple MCP clients concurrently**.
</Note>
