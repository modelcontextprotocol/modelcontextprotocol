...
<Note>
  Note that local MCP servers using the STDIO transport typically serve a single MCP client, whereas remotely hosted MCP servers using StreamableHttp may accept concurrent connections from many MCP clients.
</Note>
...
graph TB
    ...
    Client1 ---|"Many-to-one<br/>relationship"| Server1
    Client2 ---|"One-to-one<br/>relationship"| Server2
    Client3 ---|"One-to-one<br/>relationship"| Server3
```
