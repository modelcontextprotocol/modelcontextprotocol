...
relevant policy context. The provider verifies that the receipt is
fresh, intended for the current MCP server and tool, and bound to the
same action, resource, context identifiers, and request digest before
business logic runs.
