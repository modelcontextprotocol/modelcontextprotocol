...
See [Client Registration Approaches](#client-registration-approaches) for detailed information on each mechanism.
...
Pre-registration is appropriate when the client and authorization server have an existing relationship. The client can have embedded client registration information (client ID and optionally client credentials) that it uses to complete the authorization flow.

### Dynamic Client Registration

<Warning>
This is a legacy approach maintained for backwards compatibility with earlier versions of the MCP authorization specification. New implementations should prefer Client ID Metadata Documents when the authorization server supports them.
</Warning>
...
[client.dev](https://client.dev/) is a reference authorization server that supports Client ID Metadata Documents out of the box. It's designed for testing and experimentation with the CIMD approach.

<Steps>
<Step title="Create Your Client Metadata Document">
Create a JSON file with your client metadata and host it at a publicly accessible HTTPS URL. The document must include required fields:
...
Set up your MCP server to use `https://client.dev` as its authorization server. In your Protected Resource Metadata, include:
...
<Tip>
[Visual Studio Code](https://code.visualstudio.com/) supports CIMD as an MCP client, making it a convenient tool for testing your CIMD-enabled MCP servers during development.
...
<Warning>
This example uses Dynamic Client Registration (DCR), which is maintained for backwards compatibility. For new implementations, prefer [Client ID Metadata Documents](#testing-with-client-id-metadata-documents) when the authorization server supports them.
</Warning>
