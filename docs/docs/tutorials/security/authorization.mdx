...
The recommended approach is **Client ID Metadata Documents** (CIMD), where the client uses an HTTPS URL as its `client_id`. This URL points to a JSON document containing the client's metadata, which the authorization server fetches to validate the client.

This approach is ideal for the common MCP scenario where servers and clients have no pre-existing relationship. The client simply hosts its metadata at a publicly accessible URL:
...
The authorization server will fetch this document when it encounters a URL-formatted `client_id` and validate the redirect URI against the registered values. Authorization servers that support this approach will include `client_id_metadata_document_supported: true` in their metadata.

#### Pre-Registration

Alternatively, the client can be **pre-registered** with a given authorization server, in which case it can have embedded client registration information that it uses to complete the authorization flow.

#### Dynamic Client Registration (Legacy)

<Warning>
**Legacy Approach**

Dynamic Client Registration is included for backwards compatibility with earlier versions of the MCP authorization specification. New implementations **should** prefer Client ID Metadata Documents when possible.

</Warning>

For backwards compatibility, clients can use **Dynamic Client Registration** (DCR) to dynamically register with the authorization server. If the authorization server supports DCR (indicated by a `registration_endpoint` in its metadata), the client will send a request to that endpoint with its information:
...
MCP supports three client registration mechanisms to accommodate different deployment scenarios. Choose based on your needs:

| Approach | When to Use | Authorization Server Requirement |
|----------|-------------|----------------------------------|
| **Client ID Metadata Documents** | Most common; when client and server have no prior relationship | Must support `client_id_metadata_document_supported` |
| **Pre-registration** | When client and server have an existing relationship | Client credentials pre-configured |
| **Dynamic Client Registration** | Legacy; backwards compatibility | Must expose `registration_endpoint` |

### Client ID Metadata Documents (Recommended)

Client ID Metadata Documents ([draft-ietf-oauth-client-id-metadata-document-00](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document-00)) allow MCP clients to use an HTTPS URL as their `client_id`. The authorization server fetches client metadata from this URL, eliminating the need for pre-registration or dynamic registration.
...
4. The server validates the document and uses the metadata (like `redirect_uris`) to process the authorization request
...
- Host metadata at an HTTPS URL with a path component (e.g., `https://example.com/oauth/metadata.json`)
- Include required fields: `client_id` (must match the URL), `client_name`, `redirect_uris`
- Ensure the document is publicly accessible

<Tip>
**Testing Client ID Metadata Documents**

To experiment with Client ID Metadata Documents:
- Use [client.dev](https://client.dev/) as a reference authorization server that supports CIMD
- [Visual Studio Code](https://code.visualstudio.com/) supports CIMD as an MCP client for development and testing
...
Dynamic Client Registration is maintained for backwards compatibility with earlier versions of the MCP authorization specification and authorization servers that don't yet support Client ID Metadata Documents. New implementations should prefer Client ID Metadata Documents when the authorization server supports them.

</Warning>

Dynamic Client Registration ([RFC7591](https://datatracker.ietf.org/doc/html/rfc7591)) allows clients to programmatically register with an authorization server by sending a registration request containing client metadata.
...
- Authorization servers should implement appropriate controls (trusted hosts, rate limiting, etc.)
...
This example uses Dynamic Client Registration (DCR) because Keycloak has built-in support for it. While DCR is considered a legacy approach in MCP, it remains useful for testing with authorization servers that support it out of the box. For production deployments, consider using an authorization server that supports Client ID Metadata Documents, which is the recommended approach for MCP.
...
- **[OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document-00)**: Client ID Metadata Documents (recommended for client registration)
- **[RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591)**: Dynamic Client Registration (legacy)
