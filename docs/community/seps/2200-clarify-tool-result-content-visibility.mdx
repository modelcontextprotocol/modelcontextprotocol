...
1. **Tool result audiences**
   - `content` is the model-oriented representation of a tool result. It is the default field to pass into model context for and agent experiences. It MAY be designed to differ in schema from `structuredContent` to be further optimized for model-use (i.e. token-efficiency, readability, omission of empty values, etc...)
...
2. **Semantic alignment**
   ...
   - For backwards compatibility, tools that return `structuredContent` SHOULD also return a
     `content` representation of the semantically-equivalent information. JSON-serialized `structuredContent` MAY be used in `content`. 
