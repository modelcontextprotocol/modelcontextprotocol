...
Each WG has one or more **Leads** and each IG has one or more **Facilitators**.

**Requirements for all Leads/Facilitators:**

- Ability to facilitate across organizational boundaries while maintaining impartiality and alignment with the MCP principles and vision
...
**All Leads/Facilitators are responsible for:**

- Schedule and facilitate regular group/community meetings
...
- Proactively recruit and retain broad, representative membership across organizations, backgrounds, and perspectives
...
WG Leads additionally hold SEP sponsorship privileges, allowing them to formally sponsor SEPs on behalf of the group's consensus decisions.
...
If a vote fails to resolve the matter (no quorum or the result is contested), the Lead escalates to Core Maintainers following the escalation path defined below.
...
1. Lead/Facilitator documents the decision, options considered, and points of disagreement
2. Lead/Facilitator presents the escalation to the Core Maintainer group with a clear ask
...
Leads/Facilitators determine meeting frequency, format, and duration based on the group's current needs and lifecycle stage. There is no fixed cadence requirement — a WG near a specification release may meet weekly, while an IG in early exploration may meet monthly or work primarily asynchronously.
...
Leads/Facilitators should actively involve Members and Participants in operational duties such as preparing agendas, taking meeting notes, and facilitating discussions.
...
In addition to Discord, groups can establish a discussion category in the [GitHub Discussions](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/). Leads/Facilitators will be granted the appropriate roles to manage and moderate discussions.
...
**Interest Group Graduation to Working Group:**

An IG that identifies a concrete problem requiring formal specification work may petition Core Maintainers to charter a Working Group. To do so:

- The IG Facilitator documents the problem statement, proposed WG scope, and candidate Lead(s)
- The petition is submitted to Core Maintainers for review
- Core Maintainers may approve WG formation following the standard WG formation process above
