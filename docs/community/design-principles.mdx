...
These principles guide how we evaluate proposals, make tradeoffs, and evolve MCP. They reflect .lessons from building and maintaining the protocol. They are meant as guidance by and for the community when developing extensions and SEPs.
...
We are careful in considering new features in MCP, but lean into experimentation via [extensions](/community/seps/2133-extensions.mdx).
...
When someone asks "why doesn't MCP support X directly?", the answer is often "because you can build X with what MCP already provides". Extensions such as [MCP Apps](/extensions/apps/overview.mdx) capture the emerging patterns.
