...
- `Draft`: The initial proposal text, typically while searching for or working with a sponsor.
- `In-Review`: The sponsor believes the SEP is ready for broad maintainer review and community feedback.
- `Accepted`: Core Maintainers approved the proposal, pending completion of the reference implementation.
- `Final`: The reference implementation is complete and merged, and the SEP is considered normative.
- `Rejected`: The proposal will not move forward after review.
- `Withdrawn`: The author chose to stop pursuing the proposal.
- `Superseded`: A newer SEP replaces this one (the replacing SEP must be referenced explicitly).
