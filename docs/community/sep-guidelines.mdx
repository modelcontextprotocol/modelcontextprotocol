...
**The Sponsor is responsible for updating the SEP status.** This ensures that status transitions are made by someone with the authority and context to do so appropriately. The sponsor updates the `Status` field directly in the SEP markdown file and commits the change to the PR.
