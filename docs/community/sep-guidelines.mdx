...
7. **Implementation**: Prior to formal review, a reference implementation must be completed. This includes a PR updating the specification, schema, and docs (in this repo) as well as a working implementation and example in at least one first-tier SDK.

8. **Formal review**: When the SEP is ready, the sponsor updates the status to `in-review`. The SEP enters formal review by the Core Maintainers team.

9. **Resolution**: The SEP may be `accepted`, `rejected`, or returned for revision. The sponsor updates the status accordingly.

10. **Finalization**: Once accepted and incorporated into the specification, the sponsor updates the status to `final`.
...
7. **Reference Implementation** -- There is merit to the approach of reaching consensus on the specification and rationale before writing code, which is the purpose of the informal review period. For formal review, however, the principle of "rough consensus and running code" can be vital when it comes to resolving many discussions of protocol details.
...
- `in-review`: SEP proposal ready for formal review by Core Maintainers, including a reference implementation.
- `accepted`: SEP accepted as-is by Core Maintainers.
- `accepted-with-changes`: SEP accepted by Core Maintainers, but requires final wording and/or implementation changes based on feedback from the formal review.
- `rejected`: SEP rejected by Core Maintainers.
- `withdrawn`: SEP withdrawn by the author.
- `final`: SEP finalized; wording is complete, and the reference implementation is merged.
...
- A reference implementation demonstrating all aspects of the proposal
- Clear benefit to the MCP ecosystem
- Community support and consensus

Once an SEP has been accepted, and any requested changes to wording and/or the reference implementation are complete and incorporated into the repositories, the sponsor will change the status to "Final".

An SEP can also be "Rejected" or "Withdrawn". A SEP that is "Withdrawn" may be re-submitted at a later date.
