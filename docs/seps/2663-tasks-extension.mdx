...
<Warning>
  ...
  cross-task reuse, but do not by themselves guarantee isolation. Servers for
  which a given `requestState` must be consumed at most once (e.g., one-time
  redemptions) **MUST** enforce that invariant server-side.
</Warning>
