...
implementations is the *bytes*, not the *semantics* — so the construction is
...
recorded in a *tamper-evident* audit trail, but none defines what such a record is.
...
These three emit *differently shaped* records — caller-governance carries a
...
all three *conformance-anchor to one definition* without flattening their
...
outcome (an extension supplies decision *evidence* that informs the core
...
extensions, and the type id is itself a preimage *key*, so an extension's type
...
- **`runtime-security`** — converged shape; **normative registration text
  ...
  **Outcome binding (§2.1.1):** the disposition is decision *evidence* informing
  ...
  *why*; the base outcome states *what happened*.)
...
- **Normalize protected string values** to bound the coupling of integrity to
  ...
  **Registry vocabulary is exempt:** extension type ids and registered field
  ...
  canonical form verbatim — they are object *keys*, not values, and the registry
...
The *enforcement mechanism* is implementation-defined and is explicitly NOT
...
The append-only *enforcement* of §2.5 is a storage property, not a wire-observable
one — so gating it at the protocol surface would be theater. It is therefore
**attested**, but attestation MUST be *structured*: a bare "trust us" claim is not
...
threat — an *auditing organization* rewriting its own history — that is orthogonal
...
This SEP defines the *record* and its *construction*, not the policy of which
...
*construction* — an append-only, canonically-hashed, chained, independently
...
- **Type-keyed `extensions` object, not a `{type, data}` array.** The keyed-object
  ...
  type id a preimage *key* (so an extension's type cannot be swapped without
...
- **Abstract `outcome` enum, not per-layer status codes.** A closed disposition
  ...
  *evidence*, never as a competing outcome. This keeps the cross-profile anchor
...
- **Attested-but-structured append-only enforcement, not wire-gating and not
  "trust us".** Append-only *enforcement* (§2.5) is a storage property, not a
...
*interoperability primitive* — a canonical byte form that independent
...
cross-cutting *evidence* layer beneath the other proposals: admission (#2809)
...
may invoke — three different decisions, all three needing the *same* tamper-evident
...
- **Effect on referencing proposals is opt-in and non-breaking.**
  - *Admission (PR #2809).* A proposal whose tamper-evident admission record is a
  ...
  - *Runtime security (PR #2624).* This contract supplies the checkable record shape
...
*enforcement* of §2.5 is attested via §2.7. A conformance harness verifies the
*consequence* (chain verification fails after an out-of-band mutation, C-REC-4)
and the *structure* of the attestation (C-REC-7), rather than the storage
mechanism itself.
...
A hash chain provides tamper-*evidence*, not tamper-*prevention*: it guarantees
that alteration of committed records is *detectable*, not impossible. Detection
