...
This SEP introduces payment capabilities to the Model Context Protocol (MCP), enabling MCP servers to request payment for premium features or access to restricted resources. The specification defines a protocol-agnostic framework that supports multiple payment methods, with X402 Protocol v2 as the first supported payment protocol. The framework includes capability declaration, tool-based payment discovery, payment authorization flows, and secure settlement verification mechanisms.
...
2. **Control Access to Expensive Resources**: Gate access to costly third-party APIs or compute-intensive operations
3. **Support Sustainable Development**: Enable developers to monetize their MCP servers and continue improving them
...
MCP servers that support payments **MUST** include payment information directly in tool definitions returned by `tools/list`. This provides:

- **Transparent Pricing**: LLMs and clients can see costs upfront for each tool
- **Per-Tool Configuration**: Different tools can have different payment requirements
- **Multiple Payment Options**: Each tool can accept multiple protocols or payment amounts
- **Intelligent Decision Making**: LLMs can factor in costs when deciding which tools to use
...
2. Verify payment authorization through protocol-specific facilitators
3. Execute the tool only after authorization is verified
4. Settle payment via facilitator
5. Return tool result with settlement information

**Successful response with settlement:**
...
- Facilitator interface requirements (`/verify`, `/settle`, `/supported` endpoints)
...
6. Server verifies authorization via X402 facilitator
7. Server executes tool after authorization is valid
8. Server settles payment via facilitator
9. Server returns tool result with settlement confirmation (txHash)
...
- Verify all payment authorizations through designated facilitators
...
**Structured Payment Request**: Uses JSON structure in `payment.paymentRequest` field rather than base64 encoding to improve:
...
**Facilitator Integration**: The specification refers to protocol-specific documentation (e.g., X402 spec) for facilitator API details rather than duplicating them, keeping the MCP spec protocol-agnostic and maintainable.
