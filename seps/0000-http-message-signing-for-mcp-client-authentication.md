...
Today MCP authenticates clients with bearer tokens (OAuth access tokens, API keys, `MCP-Session-Id`). A bearer is, by definition, *whoever holds the string is the client.* That model has well-understood structural weaknesses that the rest of the web stack has been migrating away from:
...
- Store private keys in OS keychain / HSM / TPM where available; never in plaintext config.
...
- **User identity.** A signed request proves a specific *client instance* sent the request. Tying that to a human user remains the job of OAuth / OIDC.
- **Server-side authentication of the server to the client.** TLS / OOB pinning continues to handle this. A future SEP could profile RFC 9421 signed *responses* if there is demand.
