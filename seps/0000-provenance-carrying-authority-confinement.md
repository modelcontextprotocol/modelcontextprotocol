...
- **Server-side policy only** — inverts the direction of information: a server
  ...
  also preserves the position that MCP defines authorization *communication*
...
**Rejected — confused deputy.** `create_issue` with `{"repo": "acme/docs"}`. Inside the *host's* bound, outside the *planner's*; the meet excludes it. This is precisely what a point-of-use check cannot catch: the worker holds a valid credential, and the server would execute the call. Rejected per §4.3:
