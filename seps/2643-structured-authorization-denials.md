...
3. **Support for structured remediation data at denial**: In some cases, the server can safely share data with the client that helps with remediation. For example, OAuth 2.0 defines Rich Authorization Requests (RFC 9396) for conveying structured authorization requirements, but only as part of the authorization request flow. An IETF OAuth Working Group draft, [`draft-ietf-oauth-rar-metadata-remediation`](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/), proposes a mechanism for HTTP by defining a new `WWW-Authenticate` error code, `insufficient_authorization`, with the structured data placed in the `authorization_remediation` header error parameter, which contains a base64url-encoded JSON object. The draft is HTTP-specific, and this SEP proposes to adopt the same error-remediation pattern at the JSON-RPC layer so it applies across MCP transports.
...
An MCP server that denies a request on authorization grounds SHOULD include an authorization denial envelope in the `_meta` field of the result, under the key `io.modelcontextprotocol/authorization`. The envelope is a single object. Its shape does not vary with the transport, nor with the result that carries it.
...
| 3   | A new credential is needed and the server does not carry the remedy in a transport challenge | `CallToolResult` with `isError: true`                                                            | `available`                    | In `_meta`, with an `authorization_remediation` hint                      |
| 4   | The denial is final, or the server does not describe a remediation                           | `CallToolResult` with `isError: true`                                                            | `unavailable` or `undisclosed` | In `_meta`, with no hints                                                 |
| 5   | On HTTP, the server carries the remedy in a transport challenge                              | HTTP status line and `WWW-Authenticate`, optionally with a `CallToolResult` with `isError: true` | `available`                    | Optional. In `_meta`, with or without an `authorization_remediation` hint |

In case 5, where the server also sends the envelope, the transport challenge is authoritative for the remedy and the envelope adds the classification. Where it sends the challenge alone there is no result and therefore no envelope. Every challenge below means `reason: insufficient_authorization` and `remediation: available`, and differs only in which parameter carries the remedy.

| Challenge                                                                                                                                                         | Remedy carried in `WWW-Authenticate`       | What the client does                                                                                             |
| :---------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------- | :--------------------------------------------------------------------------------------------------------------- |
...
| `error="insufficient_authorization"` ([draft-ietf-oauth-rar-metadata-remediation](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/))   | `authorization_remediation`, base64url RAR | Decodes it and sends the `authorization_details` to the authorization server, then retries                       |
| `error="transaction_authorization_required"` ([draft-rosomakho-oauth-txn-challenge-00](https://datatracker.ietf.org/doc/draft-rosomakho-oauth-txn-challenge/00/)) | `transaction_challenge`, a JWS             | Posts the JWS to the authorization server's transaction authorization endpoint and retries with the issued token |
  ...
  "result": {
    ...
    "_meta": {
      "io.modelcontextprotocol/authorization": {
        "reason": "insufficient_authorization",
        "remediation": "available",
        "authorizationContextId": "authzctx_9e4c02b1",
...
A server knows which interactions are open to it, because a request carries the client's capabilities. When the denied request declared neither `elicitation.url` nor `io.modelcontextprotocol/tasks`, the server has no interaction to offer. It returns a `CallToolResult` with `isError: true` carrying an envelope whose `remediation` is `undisclosed`, and describes the remedy in `content`. The server MUST NOT use `unavailable` in this case, because the remedy exists and a client that declared either capability would be offered it.
...
On HTTP, where the `WWW-Authenticate` challenge describes the remediation in full, a server can answer with the challenge alone. It then returns no MCP result, so there is no envelope, and the client derives `reason` and `remediation` from the challenge as described in "Where the envelope attaches".

Where the server expresses the denial as a result, which is always the case on stdio because the MCP specification defines no authorization challenge there, the envelope MAY carry a `remediationHints` entry of type `authorization_remediation` with the following members:
...
- `authorization_reference` (string, RECOMMENDED). An opaque value generated by the server that allows the client to select an existing credential associated with equivalent authorization details, without interpreting those details. Matching only works if the value is stable across semantically equivalent authorization details. [OAuth 2.0 RAR Metadata and Error Remediation](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/) describes one approach to generating such values. Clients MUST treat the value as opaque. The server MUST NOT include this member when credentials issued for the provided `authorization_details` are intended for single use.
...
When a denied request requires authorization beyond what OAuth scopes can express, the `WWW-Authenticate` challenge can carry the structured remediation requirements directly, so the client can construct its next authorization request without further metadata discovery. This uses the error-remediation pattern defined in [OAuth 2.0 RAR Metadata and Error Remediation](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/).
...
The client may retry the failing call using an existing valid credential if it matches a provided `authorization_reference`. Otherwise the client uses the `authorization_details` from WWW-Authenticate header's `authorization_remediation` parameter, to construct an OAuth authorization request, per [RFC 9396#section-2](https://datatracker.ietf.org/doc/html/rfc9396#section-2):
  ...
  "result": {
    ...
    "_meta": {
      "io.modelcontextprotocol/authorization": {
        "reason": "insufficient_authorization",
        "remediation": "available",
        "authorizationContextId": "authzctx_c17a3f5e",
        "remediationHints": [
          {
...
```json
{
  ...
  "params": {
    ...
    "_meta": {
      "io.modelcontextprotocol/authorization-context-id": "authzctx_c17a3f5e"
    }
  }
}
```
...
A server that holds a remedy it will not describe sends the same result with `remediation` set to `undisclosed`. The `authorizationContextId` is still useful on both, because a caller who asks why the request failed needs something to quote.

## Rationale

This SEP standardizes the smallest set of facts that an MCP client and server need to communicate after an authorization denial: what classification the failure has, how the retry is correlated, and what remediation posture the server can describe. Any further extensions, whether a new remediation type or a new transport binding, can be added additively for future needs without changing the envelope itself. For example, a future `remediationHints` type could describe a Client Initiated Backchannel Authentication (CIBA) flow, in which per-operation approvals occur on a separate channel such as a push notification rather than interrupting the foreground session. A new type could equally give an existing challenge a carrier it lacks. The challenge table above carries an OAuth transaction authorization challenge only in a `WWW-Authenticate` parameter, and a future type could carry the same signed challenge in the envelope, so that remediation becomes expressible on a transport that defines no challenge layer.
...
### Information disclosure through the correlation handle

The handle gives the server a stable value for audit and telemetry, so it reaches logs and traces that outlive the exchange and that are read by parties other than the client. Servers SHOULD generate the `authorizationContextId` as an opaque value, and SHOULD NOT encode in it the operation, the resource, the policy that denied the request, or any information about the user.
...
### Integrity of remediation hints

A remediation hint tells the client what to do next, so an intermediary that can modify the response can substitute a remediation URL or alter the `authorization_details` a client will send to its authorization server. A client MUST NOT act on a remediation hint, or on the URL of a url-mode elicitation accompanying a denial, unless it has assurance that the response originated from the MCP server it addressed, and an authenticated connection to an intermediary is not that assurance. This SEP mandates no mechanism for establishing it. Deployments on HTTP MAY use [HTTP Message Signatures (RFC 9421)](https://datatracker.ietf.org/doc/html/rfc9421) with [Digest Fields (RFC 9530)](https://datatracker.ietf.org/doc/html/rfc9530).

### Client-side modification of `authorization_details`

The `authorization_details` a server places in a denial reach the authorization server by way of the client, which can change them along the way. An MCP server MUST NOT treat a token as authorizing an operation because it carries authorization details resembling the ones the denial asked for, and MUST validate the token against the operation actually requested on retry. Servers SHOULD NOT place anything in `authorization_details` that they would be unwilling for the client to see or alter.
...
- Yaron Zehavi (OAuth RAR/Payment Industry expertise; author of [OAuth 2.0 RAR Metadata and Error Remediation](https://datatracker.ietf.org/doc/draft-ietf-oauth-rar-metadata-remediation/), adopted by the IETF OAuth Working Group)
