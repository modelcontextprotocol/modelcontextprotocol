...
**Tool resolution eliminates unnecessary confirmation dialogs and permission prompts** by allowing clients to query argument-specific metadata before invoking a tool. Today, a file management tool must declare `destructiveHint: true` even though most calls are safe reads—causing users to click through warnings they learn to ignore.

This SEP introduces:

1. A `resolve` field in `Tool` definitions, **advertised statically** during `tools/list`
2. A lightweight `tools/resolve` preflight request—similar to HTTP `OPTIONS` or `HEAD`—that returns refined annotations for specific arguments

**Key design principles:**

- **Opt-in and targeted**: Clients only resolve tools that declare `resolve: true`, and only when metadata accuracy matters for UX or safety decisions
- **Fully backward compatible**: Existing servers and clients require no changes; tools remain fully functional without resolution
- **Gracefully resilient**: If resolution fails, clients fall back to the static (worst-case) annotations—the current behavior
- **Lightweight**: Resolution requests should complete in milliseconds; servers MUST NOT perform side effects or expensive computation
- **Extensible**: Returns the full `Tool` object, allowing future specifications (auth, security, trust, privacy) to leverage the same mechanism without protocol proliferation

The pattern consolidates preflight concerns into a single exchange while enabling **curated environments** (like app stores) to approve tools based on worst-case static declarations, while end users benefit from refined, argument-specific prompting.

> **Note**: While this design draws inspiration from LSP's resolve pattern, MCP tools are always complete and functional from `tools/list`. Resolution refines metadata based on intended arguments—it doesn't complete incomplete stubs.
...
1. **Confirmation fatigue**: Users see so many dialogs for safe operations that they learn to click through without reading—defeating the purpose of warnings
...
3. Servers **MUST NOT** perform side effects during `tools/resolve`—resolution is metadata-only
4. Servers **SHOULD** implement resolution as a lightweight operation completing in milliseconds (similar to HTTP `OPTIONS` or `HEAD` requests)
5. Servers **MAY** return a resolved tool with only annotation fields changed; other fields may match the original
6. Servers **MAY** provide worst-case annotations in `tools/list` as a fallback for error cases
...
#### Trust and Sensitivity Annotations (Future)

[SEP-1913 (Trust and Sensitivity Annotations)](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/1913) proposes metadata for tracking data sensitivity, privacy, and provenance as it flows through MCP. Tool resolution can enhance this with **argument-specific sensitivity metadata**:
...
**Use Case: Curated App Stores and Registries**

Tool resolution supports a layered approval model for curated environments:

1. **App store/registry approval**: Reviews the static `tools/list` definitions with worst-case annotations. A tool declaring `destructiveHint: true` can be approved with appropriate warnings and documentation.

2. **Runtime UX optimization**: When users actually invoke the tool, clients call `tools/resolve` and discover that specific operations (e.g., read-only queries) don't require confirmation dialogs.

This separation means:

- **Curators** can make conservative approval decisions based on maximum potential impact
- **Users** avoid confirmation fatigue for safe operations within approved tools
- **Trust boundaries** are maintained—resolved annotations can never exceed what the static declaration permits
...
3. Implementations should be computationally lightweight to prevent DoS—resolution should complete in milliseconds, not seconds
...
- **[SEP-1913 (Trust and Sensitivity Annotations)](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/1913)**: Proposes metadata for tracking data sensitivity, privacy, and provenance. Tool resolution can return argument-specific trust metadata before execution, enabling pre-execution policy decisions.
...
- **[SEP-711 (Trust Annotations)](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/711)**: Original discussion of sensitivity and provenance annotations for data flowing through MCP, now formalized in SEP-1913.
