# SEP-0000: Attested Tool-Server Admission (ATSA)
...
- **PR**: (to be assigned — rename this file to `NNNN-attested-tool-server-admission.md` once the PR number is known)
...
MCP standardizes how a host and a tool server exchange messages, but not *trust*:
...
layer: a server publishes a small, offline-signed *clearance assertion* at a
...
may use the server; neither answers the missing third question — *is this server
one the host is authorized to use as a tool provider, and at what sensitivity?*
...
1. **Well-known resource.** The server **SHOULD** serve a *Server Attestation
   Document* (SAD) at `/.well-known/mcp-attestation` on the same origin as its
...
the absence of both as *unattested*, applying its configured posture (see
...
Before treating a connection as *admitted*, a host **MUST** evaluate the
...
tool absent from it *before* any network dispatch, regardless of what
...
The mechanism can be implemented above MCP today — but *additive* is not
*interoperable*. Bolted on by one vendor, attestation secures only that vendor's
...
- **Offline, detached signature.** Admission requires no online call to a third
  ...
  infrastructure.
- **Well-known URI for discovery.** A host can evaluate admission *before*
...
- **Leave it to each vendor / keep it an extension only.** Rejected: additive ≠
  interoperable (above). Extensions are the right place to *experiment*; this
...
- **In-band capability only, no well-known document.** Rejected: discovery must be
  possible *before* a connection is trusted, but `initialize` already implies a
...
- **OAuth-style bearer token.** Rejected: that is *user*-authorization ("may this
...
- **Online (OCSP-style) revocation as a v1 requirement.** Deferred: it adds an
  ...
  online revocation is future work (see *Open questions*).
...
- **Convergence over choice.** One signed-attestation format, one verification
  ordering — so "this server is attested" becomes a claim *any* host can check and
  a server publishes *once*, instead of N mutually unintelligible vendor dialects.
...
- **Composability over specificity** *(objection addressed).* A reviewer may ask
...
- **Stability over velocity** *(objection addressed).* Because the addition is
...
- **Capability over compensation.** ATSA does not compensate for a temporary model
  weakness. A more capable model that is prompt-injected is *more* dangerous, not
...
are deployment policy (see *Trust establishment*), and a compromised signer key
...
distribution — <https://github.com/enclawed/enclawed-oss> — at
...
SHOULD/SHOULD NOT in this *Specification* to a check ID. This SEP is written to
...
- Companion preprint: *Attested Tool-Server Admission: A Security Extension to the
  Model Context Protocol* — archived at Zenodo, doi:10.5281/zenodo.20349263
...
- Reference implementation: <https://github.com/enclawed/enclawed-oss> —
