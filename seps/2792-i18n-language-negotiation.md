...
- When a client includes `io.modelcontextprotocol/acceptLanguage` in
  `params._meta`, it **MUST** also set the HTTP `Accept-Language` header on
  the corresponding POST to the identical value.
- A client **MAY** set `Accept-Language` without the `_meta` field (e.g. a
  pure HTTP client). Servers **SHOULD** treat a bare `Accept-Language`
  header the same as the `_meta` field.
...
- On the Streamable HTTP transport, the `Content-Language` response header
  **MUST** mirror this value when an error response is returned, exactly as
  it does for successful responses.
...
- When a server emits `io.modelcontextprotocol/contentLanguage` in the
  response `_meta`, it **MUST** also set the HTTP `Content-Language`
  response header to the identical value.
- Streaming responses (`text/event-stream`) **MUST** include
...
### Why mirror to HTTP headers (and require consistency)

[SEP-2243] established the principle that fields useful to network
intermediaries belong in headers, and that header/body disagreement is a
correctness hazard. SEP-2243 defines the mismatch rule and the
`-32001 HeaderMismatch` JSON-RPC error code over the small set of headers
it introduces. This SEP **extends** that rule and that error code to the
standard HTTP `Accept-Language` / `Content-Language` headers, applying the
same correctness contract to the broader category of headers that are
genuinely useful to:
...
A mismatch indicates a buggy client or a tampering intermediary, and the
server **MUST** reject it with `-32001 HeaderMismatch` (HTTP `400 Bad
Request`).
...
- The strict header/body mismatch rule applies only when _both_ are present
  on the same request, mirroring SEP-2243's behavior.
...
- **Mismatch attacks.** This SEP extends the strict-mismatch rule from
  [SEP-2243] (and its `-32001 HeaderMismatch` JSON-RPC error code) to the
  standard `Accept-Language` / `Content-Language` headers, broadening the
  same anti-tampering contract to a much larger surface of intermediaries
  (CDNs, edge i18n services) that routinely touch these headers.
...
2. Streamable HTTP transport mirroring in both directions:
   ...
   requests, `_meta[contentLanguage]` to/from `Content-Language` on
   responses, and the strict-mismatch rule returning HTTP `400`.
...
5. Tests: 20 unit tests for the helpers and `negotiateLanguage` (quality
   values, wildcards, fallback), 3 HTTP integration tests (header
   mirroring, mismatch returns 400, `Content-Language` echoed), and 2
   stdio integration tests, including the critical proof that two
...
All builds, typechecks, lints, and tests pass on the implementation
branch.

Implementation notes called out in the PR for reviewers:

- The mismatch error currently reuses `SdkErrorCode.SendFailed`. Per
  reviewer feedback, this will be updated to the
  `-32001 HeaderMismatch` code defined by [SEP-2243] (now in the draft
  spec) as part of unifying with the SEP-2243 pattern in the SDK.
- This SDK does not yet implement the broader [SEP-2243] mismatch
  pattern; the language-specific mismatch is implemented standalone
  with a `TODO` pointing at SEP-2243 so the two can be unified when
  SEP-2243 lands here.
- `Content-Language` is set as an HTTP response header only in JSON
  response mode; for SSE streaming the headers are flushed before
  response content is known, so the `_meta` field carries the value in
  that case. The `_meta` field remains authoritative either way.
- Error-response localization (`error.data._meta` carrying
  `contentLanguage`, mirrored to the `Content-Language` response header)
  is on the open work list for the reference implementation, following
  the clarification added to this SEP.
...
4. A header/body mismatch on a Streamable HTTP request being rejected
   with JSON-RPC error `-32001 HeaderMismatch` and HTTP `400`.
5. A localized error response carrying
   `error.data._meta['io.modelcontextprotocol/contentLanguage']`,
   mirrored to `Content-Language` on HTTP.
6. Per-request language switching on the same connection (notably
