...
- A server that selected a language in response to `acceptLanguage`, or that
  ...
  include `io.modelcontextprotocol/contentLanguage` in `result._meta` (or
  `_meta` of the error object on error responses).
...
- Servers that process the request body **MUST** reject requests where the
  header and the `_meta` field are both present and disagree, with the same
  error semantics as the mismatch rule in [SEP-2243] (HTTP `400 Bad
Request`).
...
When a Streamable HTTP request batches multiple JSON-RPC messages with
differing `acceptLanguage` values, the HTTP header **MUST** carry the union
of the language ranges (the server still resolves per-message via the
`_meta` field). This preserves header visibility for intermediaries
without losing per-message granularity.
...
correctness hazard. Language preference is useful to:
...
The strict-mismatch rule from SEP-2243 is reused unchanged: a mismatch
indicates a buggy client or a tampering intermediary, and the server
**MUST** reject it.
...
- **Mismatch attacks.** The strict-mismatch rule (inherited from
  SEP-2243) prevents an intermediary from rewriting one side without the
  other.
...
- The mismatch error currently reuses `SdkErrorCode.SendFailed` since
  the SDK has no `InvalidRequest` variant; revisit if a more specific
  code is added.
...
2. **`Vary` header guidance.** Should this SEP mandate
   ...
   RFC 9111.)
3. **Coordination with SEP-1809.** Requires explicit sign-off from the
   SEP-1809 author before merging the subsumption claim.
