...
`Accept-Language` field whose value agrees exactly with it.

#### Exact field value agreement

Language _matching_ is semantic: [RFC 4647] over the parsed value, with
case-insensitive tags, weights, and the wildcard as [RFC 9110 §12.5.4] defines
them. Mirror _agreement_ is a different question and is decided by exact
equality of the two exposed field values. It is **not** semantic
`Accept-Language` or `Content-Language` equivalence, and it is **not** equality
of the bytes on the wire, which differ between HTTP versions.

A receiver determines agreement as follows:

1. Take the decoded `_meta` string from the parsed JSON body. The
   implementation **MUST NOT** trim, case-fold, reorder, or otherwise
   normalize it.
2. Take the HTTP field value after normal field parsing. Optional whitespace
   surrounding a field line is not part of the field value
   ([RFC 9110 §5.5]). If the HTTP API exposes repeated field lines
   separately, join every value in received order with a comma and a space
   (`", "`), per the consistency recommendation in [RFC 9110 §5.3]; if it
   exposes the complete combined field value, use that directly.
3. Compare the two strings character for character. Implementations
   **MUST NOT** parse, normalize, or re-serialize either value beyond the join
   in step 2. Both grammars are ASCII, so this is equivalent to comparing
   their ASCII encodings octet for octet.

Implementations **MUST** use an HTTP API that exposes either every field line
value or the complete combined field value. An accessor that returns only the
first value of a repeated field is not sufficient, because it can hide a
conflicting field line.

Senders **MUST** emit exactly one field line for each mirrored language field,
so that no legal recombination of repeated field lines can change the value a
receiver computes.

Internal whitespace, tag casing, range order, and `q`-value spelling are
therefore significant, even where HTTP treats the values as equivalent:
`en-US` and `en-us`, `en;q=0.9` and `en;q=0.900`, and `en-US,en` and
`en-US, en` are each pairs that do not agree.

#### Presence and mismatch

The payload/header agreement rule from [SEP-2243] and the current
...
the canonical `_meta` value. If both are present and do not agree, the server
**MUST** reject the request with HTTP `400 Bad Request` and `HeaderMismatch`
(`-32020`). A bare header without the canonical `_meta` field does not create
an MCP preference.

Agreement is evaluated before grammar validation, so the outcome never depends
on whether either value happens to be well formed.

For a JSON response carrying `contentLanguage`, including an error response
carrying it in `error.data._meta`, the server **MUST** mirror it into
`Content-Language`. If a client receives both values and they do not agree,
...
message on that stream. If it does send one, every `contentLanguage` carried
by a message on that stream **MUST** agree with it. Each localized response or
notification still carries its own canonical `_meta` value.
...
JSON body unchanged will cause `HeaderMismatch`. Normalizing whitespace,
case-folding tags, reordering ranges, canonicalizing `q` values, and splitting
or recombining field lines each change the value a receiver computes, even
though HTTP permits them. An intermediary on an MCP path must preserve each
mirrored language field exactly or remove it; removal is always safe, because
a missing `Accept-Language` is tolerated. This is similar to the
routing-integrity requirement already used by Streamable HTTP mirroring.
...
matcher. A malformed canonical value is ignored as an absent preference and
**MUST NOT** on its own produce an error. Because agreement is checked first
and compares the values as strings, a present HTTP mirror still has to agree
exactly, whether or not either value is well formed.
...
7. Exact agreement edge cases, each of which **MUST NOT** agree: `en-US` and
   `en-us`; `en-US,en;q=0.9` and `en-US, en;q=0.9`; `en;q=0.9` and
   `en;q=0.900`. A field line `Accept-Language:  en ` and a `_meta` value of
   `en` **MUST** agree, because the surrounding whitespace is not part of the
   field value. Two field lines valued `en-US` and `en;q=0.9` combine to
   `en-US, en;q=0.9` and **MUST** agree with exactly that `_meta` value.
8. SSE and notification reporting through per-message `_meta`.
9. MCP and HTTP cache separation across language preferences, including
   `ttlMs`, `cacheScope`, `Vary: Accept-Language`, and the stripped-header
   shared-cache case.
10. HTTP language-preference privacy guidance.
