...
- The `_meta` field is **authoritative**. The HTTP `Accept-Language`
  header is a best-effort mirror that may be stripped, normalized,
  rewritten, or consumed by intermediaries (CDNs, edge i18n routers,
  reverse proxies, vendor language-lookup VCL). Servers **MUST NOT**
  reject a request because the header and `_meta` field disagree; if
  `_meta` is present, the server uses it and ignores the header.
...
- CloudFront strips `Accept-Language` from forwarded requests by default
  unless explicitly configured to preserve it.
- Fastly's `accept.language_lookup()` VCL and Varnish's `vmod_accept` are
  the recommended way to get the per-language caching benefit cited
  above; both rewrite `Accept-Language` to a single normalized tag
...
Additionally, RFC 9110 does not define a single canonical serialization
for `Accept-Language`: whitespace, case, `q` value trailing zeroes, and
list field splitting are all under-specified, so "the header and the
body field disagree" is not even a well-defined comparison.
...
- The reference implementation does **not** reject header/body
  mismatches; per the final design, `_meta` is canonical and the
  header is a best-effort mirror. Earlier revisions of this SEP
  proposed a `-32001 HeaderMismatch` rule lifted from [SEP-2243];
  see the PR discussion for why that was dropped (intermediary
  header rewriting is expected behavior, and `Accept-Language`
  has no canonical byte-equality form).
...
- Error-response localization (`error.data._meta` carrying
  ...
  on JSON responses) is implemented and exercised in the example
