...
- **Author:** Rohith Gajawada (@RohithGajawada45), Mahathi Kanduri (@Mahathi04)
...
Binding of the compensating operation's parameters follows OpenAPI 3.0's `Link` object runtime-expression syntax (`$response.body#/field`, `$response.header.<name>`, etc.), applied to the *original* tool call's response. For example, a `charge_payment` tool that returns `{"transactionId": "TXN-123", "amount": 59.97}` can declare:
...
When a workflow step fails *after* an earlier step has already committed a durable side effect, retrying the earlier operation does not undo it — retrying `charge_payment` does not un-charge a card that was already charged; it only guards against charging it twice. A compensating operation is required to restore the previous state. The two mechanisms are complementary, not competing, and this proposal is scoped narrowly to the second case, which today has no representation in `ToolAnnotations` at all.
