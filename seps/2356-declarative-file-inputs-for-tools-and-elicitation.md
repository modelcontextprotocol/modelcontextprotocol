...
### Large files: URL-mode elicitation
...
single JSON-RPC message that both sides must buffer in full. For files above
the server's `maxInlineSize` threshold, this SEP does not introduce a new
upload protocol. Servers **SHOULD** instead obtain the file via [URL-mode
elicitation][url-elicit], which already provides an out-of-band browser flow
where the upload protocol (presigned, resumable, chunked, authenticated) is
entirely server-controlled.

The flow on each surface, using only existing primitives:

**Tool surface.** The host shows a file picker at the human-in-the-loop step
(per [Host integration on the tool surface](#host-integration-on-the-tool-surface)).
If the selected file's decoded size exceeds `maxInlineSize`, the host
dispatches `tools/call` with the `mcpFile` argument **absent**. The server,
seeing the slot unfilled, sends a nested `elicitation/create` with
`mode: "url"` whose `url` is a server-hosted upload page. The user uploads in
the browser; the server receives the file directly, signals completion via
`notifications/elicitation/complete`, and resumes the tool call from session
state. The `mcpFile` slot never carries the large file on the wire.

**Elicitation surface.** A server that needs a large file **SHOULD** issue a
URL-mode elicitation directly rather than a form-mode one with `mcpFile`,
since the file would not fit the form-mode `ElicitResult.content` string slot
either. A server **MAY** issue form-mode `mcpFile` first and, on receiving a
`file_too_large` cancellation or no response, re-elicit in URL mode.

Servers that expect large files **SHOULD** declare `maxInlineSize` so hosts
can skip encoding a payload that will not be accepted inline. Servers that set
`maxInlineSize` **MUST** declare the `elicitation.url` server capability or
otherwise be prepared to handle the slot arriving absent.
...
For files too large to embed inline,
[Large files: URL-mode elicitation](#large-files-url-mode-elicitation) reuses
the existing URL-mode mechanism rather than introducing an upload protocol
into MCP. Upload protocols vary widely (presigned PUT, resumable, chunked
multipart, authenticated endpoints); a generic descriptor that covered all of
them would either be incomplete or would amount to specifying a new transport.
URL-mode keeps the upload protocol server-side where it belongs, and `file://`
remains available for local stdio servers that share a filesystem.

### Why not a presigned-upload descriptor?

An alternative considered was a `FileUploadTarget` descriptor
(`{url, method, headers, fields, expiresAt, reference}`) carried on form-mode
elicitation, modelled on S3/GCS/Azure presigned URLs. It was rejected because
it only covers single-shot presigned object storage: the credential-forgery
defense (clients refuse `Authorization` in `headers`) means it cannot reach
authenticated server-owned endpoints; it cannot express resumable protocols
(tus, GCS resumable) or multipart-chunked uploads; and any server needing
those would fall through to URL-mode anyway. URL-mode already exists, already
handles every upload protocol the server cares to implement, and adds no new
schema surface.
...
- **`mcpFile` with `data:` (push, inline):** The client knows at tool-listing
  time which arguments are files, encodes them inline, and sends them with the
  call. Suited to small-to-medium payloads where inline transfer is practical.
- **URL-mode elicitation (pull, out-of-band):** The server directs the user's
  browser to an upload page it controls. Suited to large payloads, and the
  path this SEP specifies for files above `maxInlineSize`.
- **`file://` (local only):** When client and server share a filesystem on a
  stdio transport.

A server selects the path based on the file's size and the client's declared
