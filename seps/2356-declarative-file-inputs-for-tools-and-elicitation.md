...
interface FileInputDescriptor {
   ...
   * a data URI. Servers MUST reject larger payloads with the "file_too_large"
...
arguments. Servers **MUST** reject any other scheme with the
`file_scheme_unsupported` structured error reason and **MUST NOT** dereference
it.
...
Servers reporting a structured error for an `mcpFile` argument **MUST** set
`reason` to one of the following values (servers **MAY** define additional
values for conditions not listed here):

| `reason`                  | Meaning                                               |
| ------------------------- | ----------------------------------------------------- |
| `file_too_large`          | Decoded content size exceeds `maxSize`.               |
...
| `file_scheme_unsupported` | Value is not a `data:` URI.                           |

#### Tool calls

Following [SEP-1303][sep-1303], servers **SHOULD** report these as
...
```json
{
  ...
  "result": {
    "isError": true,
    "content": [{ "type": "text", "text": "File exceeds maximum size" }],
    "structuredContent": {
...
- A sample server exposing an image-processing tool with `mcpFile`, validating
  `maxSize` and `accept` on receipt and emitting the structured error reasons.
