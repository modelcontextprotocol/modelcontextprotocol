...
// New RequestParams type includes fields for retried requests.
...
| ClientRequest | ServerResult | IncompleteResult Supported |
| ------------- | -------------------------- | -------------------------- |
...
Servers MUST NOT send `IncompleteResult` responses on any other Client Requests. The below table represents what `ClientRequest`s this excludes at the writing of this SEP.

| ClientRequest | IncompleteResult Supported |
| ------------- | -------------------------- |
| `PingRequest` | No |
...
<details> <summary>Click to expand Example Flow for Ephemeral Tools</summary>
...
<details><summary>Click to expand ADO Custom Rules Example</summary>
...
2. **Client Behavior:**
   - If a client receives an `IncompleteResult` message,
   ...
   - If a client receives a `IncompleteResult` message
     ...
     about the `requestState` contents. If the `IncompleteResult` does
...
<details><summary>Click to expand Example Flow for Persistent Tools</summary>
