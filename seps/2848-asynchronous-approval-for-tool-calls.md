...
traceability file covering the MCP-observable normative statements (returning a task
on a requestable denial; never surfacing authorization artifacts; conveying
denied-versus-executed in the `CallToolResult` body with a `denied-not-executed`
disposition for a terminal denial; at-most-once execution with best-effort
server-side dedup and a new request after a terminal denial; and submission input via
the task input channel), is required for Final. Server-internal behavior such as re-evaluation is referenced
from the AuthZEN profile and is not MCP-observable. Not yet built.
