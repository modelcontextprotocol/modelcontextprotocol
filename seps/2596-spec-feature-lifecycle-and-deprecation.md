...
Removal does not require its own SEP. The decision to deprecate, taken through the deprecation SEP,
already commits the project to retirement and fixes the earliest removal; a second SEP to carry
out a retirement that has already been argued and scheduled is ceremony rather than deliberation.
Removal is therefore a Core Maintainer decision taken during release preparation, on or after the
earliest removal, under the [governance decision process][governance-decisions]. Before removing a
feature the Core Maintainers MUST confirm, and record alongside the changelog entry below, that:

1. The minimum deprecation window has elapsed (the earliest removal has been reached).
2. The migration target named in the deprecation SEP, if any, remains Active in the revision from
...
states](#feature-states)) and **shortening** it below the twelve-month floor ([Expedited
removal](#expedited-removal)) each require a SEP, because each overturns a decision the deprecation
SEP already made. A routine removal on schedule overturns nothing, so the release-preparation
decision and the confirmations above are sufficient.

A feature MAY remain Deprecated indefinitely. The earliest removal sets the point at which removal
becomes permissible; it does not oblige removal to happen. Where ecosystem migration is slower than
anticipated the Core Maintainers may simply not take the removal decision; the deprecation notice,
the [registry](#the-deprecated-registry) entry, and the earliest removal remain as the planning
signal.
