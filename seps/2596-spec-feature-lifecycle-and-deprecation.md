...
transports, schema types, and normative behavioural requirements. It does not govern the
...
Throughout this policy, a revision is "released" on the date it is promoted to Current under the
[versioning guide][versioning].

When the deprecation SEP reaches Final, the following changes land in the draft specification
(`schema/draft/` and `docs/specification/draft/`):
