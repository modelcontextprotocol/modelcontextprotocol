...
and uses URI strings, including RFC 2397 `data:` URIs, as the baseline input value. This
SEP adds file-transfer URIs, capability negotiation for transfer modes, and control-plane
methods for preparing uploads and resolving downloads.
...
4. File collection **SHOULD** happen before the main request is sent, rather than through
   a follow-up elicitation or URL-mode loop.
...
- Clients **MUST** use `files/prepareUpload` and send the returned file URI when
  `transferModes` is `["upload"]`.
- Servers that returned a file URI from `files/prepareUpload` **MUST** accept that URI in
...
This SEP supports elicitation by allowing the elicitation response value for an
`x-mcp-file` field to be either a SEP-2356 `data:` URI or, when out-of-band transfer is
used, a file URI prepared through `files/prepareUpload`. It does not add a new
elicitation mode.
...
- `transferModes`: optional list of allowed transfer modes for the field. If omitted,
  ...
  allows `files/prepareUpload` and a file URI.
...
defined by this SEP **MUST NOT** require `resources/read`. The optional resource-read
integration in the next section is only for resources whose contents are file-backed; it
is not required for tool-result file outputs.
...
For out-of-band uploads, metadata is exchanged during `files/prepareUpload`. The server
returns a `FileValue` there so the client has a stable URI plus display metadata, and the
server can bind that URI to the uploaded bytes and validation state. The eventual
`tools/call` or elicitation response only needs to carry the URI string.
...
`files/getDownload` or the upload descriptor returned by `files/prepareUpload`.
