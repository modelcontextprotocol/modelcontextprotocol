...
If a server requires client capabilities the client has not declared, the
server MUST return a JSON-RPC error. For HTTP, the response status code MUST
be `400 Bad Request`. The error data identifies the required capabilities so
the client knows which capabilities are missing.
